...
//+------------------------------------------------+
//| Входные параметры индикатора 3LineBreak        |
//+------------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H12; //таймфрейм индикатора 3LineBreak
input int Lines_Break=3;                          //период индикатора
...
int OnInit()
  {
//---- получение хендла индикатора 3LineBreak
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"3LineBreak",Lines_Break);
   if(InpInd_Handle==INVALID_HANDLE) Print(" Не удалось получить хендл индикатора 3LineBreak");

//---- инициализация переменной для хранения периода графика в секундах  
   TimeShiftSec=PeriodSeconds(InpInd_Timeframe);

//---- инициализация переменных начала отсчёта данных
   min_rates_total=int(Lines_Break+3+SignalBar+1);
//----
   return(0);
  }
//+------------------------------------------------------------------+
   ...
   if(!SignalBar || NB.IsNewBar(Symbol(),InpInd_Timeframe) || Recount) // проверка на появление нового бара
     {
      ...
      if(CopyBuffer(InpInd_Handle,2,SignalBar,2,TrendValue)<=0) {Recount=true; return;}
