...
//+----------------------------------------------+
//| Входные параметры индикатора iStochKomposter |
//+----------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H1; //таймфрейм индикатора iStochKomposter
uint ATR_Period=14;
...
int OnInit()
  {
//---- получение хендла индикатора iStochKomposter
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"iStochKomposter",ATR_Period,KPeriod,DPeriod,Slowing,STO_Method,Price_field,UpLevel,DnLevel);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора iStochKomposter");
      return(INIT_FAILED);
     }
