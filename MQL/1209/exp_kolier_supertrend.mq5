...
//+------------------------------------------------+
//| Kolier_SuperTrend indicator input parameters   |
//+------------------------------------------------+
...
int OnInit()
  {
//---- getting the Kolier_SuperTrend indicator handle
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"Kolier_SuperTrend",3,ATR_Period,ATR_Multiplier,0);
   if(InpInd_Handle==INVALID_HANDLE) Print(" Failed to get handle of Kolier_SuperTrend indicator");

//---- initialization of a variable for storing a chart period in seconds  
   TimeShiftSec=PeriodSeconds(InpInd_Timeframe);

//---- initialization of variables of the start of data calculation
   min_rates_total=int(3+2+1+2+SignalBar);
//----
   return(0);
  }
//+------------------------------------------------------------------+
   ...
   if(!SignalBar || NB.IsNewBar(Symbol(),InpInd_Timeframe) || Recount) // checking for a new bar
     {
      ...
      else if(UpTrend[0]) SELL_Close=true;
      ...
      else if(DnTrend[0]) BUY_Close=true;
     }
