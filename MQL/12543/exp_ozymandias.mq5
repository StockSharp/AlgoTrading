...
//+----------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H4; // Таймфрейм индикатора Ozymandias
input uint Length=2;
...
//--- получение хендла индикатора Ozymandias
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"Ozymandias",Length,MAType,0);
   ...
   if(!SignalBar || NB.IsNewBar(Symbol(),InpInd_Timeframe) || Recount) // проверка на появление нового бара
     {
      ...
      if(Value[1]==0)
        {
         if(SellPosOpen) if(Value[0]==1) SELL_Open=true;
