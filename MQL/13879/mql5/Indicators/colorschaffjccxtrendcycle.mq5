//+------------------------------------------------------------------+
//|                                    ColorSchaffJCCXTrendCycle.mq5 |
//|                                  Copyright © 2011, EarnForex.com |
//|                                        http://www.earnforex.com/ |
//+------------------------------------------------------------------+ 
#property copyright "Copyright © 2011, EarnForex.com"
#property link      "http://www.earnforex.com"
#property description "Schaff Trend Cycle - Cyclical Stoch over Stoch over JCCX MACD."
...
#property indicator_color1 clrRed,clrCrimson,clrMediumVioletRed,clrMaroon,clrMediumBlue,clrBlue,clrDodgerBlue,clrDeepSkyBlue
//---- линия индикатора - сплошная
#property indicator_style1 STYLE_SOLID
//---- толщина линии индикатора равна 2
#property indicator_width1 2
//---- отображение метки индикатора
#property indicator_label1 "Schaff JCCX Trend Cycle"
//+-----------------------------------------------+
...
//+-----------------------------------------------+
input uint Fast_JCCX = 23; // Период быстрого JCCX
input uint Slow_JCCX = 50; // Период медленного JCCX
...
//--- получение хендла индикатора MA 1
   Ind1_Handle=iCustom(NULL,0,"JCCX",Fast_JCCX,JPhase,Smooth,IPC,0);
   if(Ind1_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора JCCX 1");
      return(INIT_FAILED);
     }
//--- получение хендла индикатора MA 2
   Ind2_Handle=iCustom(NULL,0,"JCCX",Slow_JCCX,JPhase,Smooth,IPC,0);
   if(Ind2_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора JCCX 2");
      return(INIT_FAILED);
     }
...
//---- инициализация переменной для короткого имени индикатора
   string shortname;
   StringConcatenate(shortname,"Schaff JCCX Trend Cycle( ",Fast_JCCX,", ",Slow_JCCX,", ",Cycle," )");
...
//---- объявление переменных с плавающей точкой  
   double fastJCCX[],slowJCCX[],LLV,HHV;
...
//---- копируем вновь появившиеся данные в массивы
   if(CopyBuffer(Ind1_Handle,0,0,to_copy,fastJCCX)<=0) return(RESET);
   if(CopyBuffer(Ind2_Handle,0,0,to_copy,slowJCCX)<=0) return(RESET);
//---- индексация элементов в массивах как в таймсериях  
   ArraySetAsSeries(fastJCCX,true);
   ArraySetAsSeries(slowJCCX,true);
   ...
   for(bar=limit; bar>=0 && !IsStopped(); bar--)
     {
      ...
      Bar1=Count[1];
      //----
      MACD[Bar0]=fastJCCX[bar]-slowJCCX[bar];
      //----
