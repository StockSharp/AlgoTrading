//+------------------------------------------------------------------+
//|                                    ColorSchaffTriXTrendCycle.mq5 |
//|                                  Copyright © 2011, EarnForex.com |
//|                                        http://www.earnforex.com/ |
//+------------------------------------------------------------------+ 
#property copyright "Copyright © 2011, EarnForex.com"
#property link      "http://www.earnforex.com"
#property description "Schaff Trend Cycle - Cyclical Stoch over Stoch over TriX MACD."
...
#property indicator_label1 "Schaff TriX Trend Cycle"
//+-----------------------------------------------+
...
//---- инициализация переменных начала отсчета данных
   min_rates_1=int(Fast_TriX);
   min_rates_2=int(Slow_TriX);
   min_rates_total=3*MathMax(min_rates_1,min_rates_2)+int(Cycle)+1;
//--- получение хендла индикатора MA 1
   Ind1_Handle=iTriX(Symbol(),PERIOD_CURRENT,Fast_TriX,AppliedPrice);
...
//--- получение хендла индикатора MA 2
   Ind2_Handle=iTriX(Symbol(),PERIOD_CURRENT,Slow_TriX,AppliedPrice);
...
//---- инициализация переменной для короткого имени индикатора
   string shortname;
   StringConcatenate(shortname,"Schaff TriX Trend Cycle( ",Fast_TriX,", ",Slow_TriX,", ",Cycle," )");
...
//---- объявление переменных с плавающей точкой  
   double fastTriX[],slowTriX[],LLV,HHV;
...
//---- копируем вновь появившиеся данные в массивы
   if(CopyBuffer(Ind1_Handle,0,0,to_copy,fastTriX)<=0) return(RESET);
   if(CopyBuffer(Ind2_Handle,0,0,to_copy,slowTriX)<=0) return(RESET);
//---- индексация элементов в массивах как в таймсериях  
   ArraySetAsSeries(fastTriX,true);
   ArraySetAsSeries(slowTriX,true);
   ...
   for(bar=limit; bar>=0 && !IsStopped(); bar--)
     {
      ...
      Bar1=Count[1];
      //----
      MACD[Bar0]=fastTriX[bar]-slowTriX[bar];
      //----
