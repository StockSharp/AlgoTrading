//+------------------------------------------------------------------+
//|                                Exp_ColorSchaffTriXTrendCycle.mq5 |
...
//+------------------------------------------------------------+
//| Входные параметры индикатора ColorSchaffTriXTrendCycle      |
//+------------------------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H4;  // Таймфрейм индикатора ColorSchaffTriXTrendCycle
...
int OnInit()
  {
//---- получение хендла индикатора ColorSchaffTriXTrendCycle
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"ColorSchaffTriXTrendCycle",Fast_TriX,Slow_TriX,AppliedPrice,Cycle,HighLevel,0,LowLevel);
   if(InpInd_Handle==INVALID_HANDLE)
    {
     Print(" Не удалось получить хендл индикатора ColorSchaffTriXTrendCycle");
     return(INIT_FAILED);
     }
//---- инициализация переменной для хранения периода графика в секундах  
   TimeShiftSec=PeriodSeconds(InpInd_Timeframe);
//---- инициализация переменных начала отсчета данных
   int min_rates_1=int(Fast_TriX);
   int min_rates_2=int(Slow_TriX);
