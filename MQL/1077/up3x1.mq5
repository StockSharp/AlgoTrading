...
#property description "Rewritten from MQL4. Link to the original publication - http://codebase.mql4.com/ru/332, author: izhutov (http://www.mql4.com/ru/users/izhutov)"
...
input int    TakeProfit         = 150;   /*TakeProfit*/        // Take Profit in points
...
//+------------------------------------------------------------------+
bool OpenBuy(){
   return(f1[0]<m1[0] && m0[0]<f0[0] && m1[0]<s1[0] && f0[0]<s0[0]);
   // The fast MA crosses the middle MA upward, while the fast MA and the middle MA are below the slow MA
}
//+------------------------------------------------------------------+
...
bool OpenSell(){
   return(f1[0]>m1[0] && m0[0]>f0[0] && m1[0]>s1[0] && f0[0]>s0[0]);
   // The fast MA crosses the middle MA downward, while the fast MA and the middle MA are above the slow MA   
}
