...
//+----------------------------------------------+
//| Входные параметры индикатора ColorXdinMA     |
//+----------------------------------------------+
...
input uint Length_main=10; // Глубина main усреднения
input uint Length_plus=20; // Глубина plus усреднения
...
int OnInit()
  {
//---- получение хендла индикатора ColorXdinMA
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"ColorXdinMA",MA_Method1,Length_main,Length_plus,PhaseX,IPC,0,0);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора ColorXdinMA");
      return(INIT_FAILED);
     }
//---- инициализация переменной для хранения периода графика в секундах  
   TimeShiftSec=PeriodSeconds(InpInd_Timeframe);
//---- инициализация переменных начала отсчета данных
   int StartBars1=GetStartBars(MA_Method1,Length_main,PhaseX);
   int StartBars2=GetStartBars(MA_Method1,Length_plus,PhaseX);
