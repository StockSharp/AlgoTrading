...
int OnInit()
  {
//---- получение хендла индикатора ColorX2MA_Digit
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"ColorX2MA_Digit","ColorX2MA_Digit",MA_Method1,Length1,Phase1,MA_Method2,Length2,Phase2,IPC,0,0,Digit,true,clrGray);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора ColorX2MA_Digit");
      return(INIT_FAILED);
     }
   ...
   if(!SignalBar || NB.IsNewBar(Symbol(),InpInd_Timeframe) || Recount) // проверка на появление нового бара
     {
      ...
      if(Value[1]==2)
        {
         if(SellPosOpen && Value[0]<2) SELL_Open=true;
