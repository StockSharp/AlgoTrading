...
//+----------------------------------------------+
//| Входные параметры индикатора iRSISign        |
//+----------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H1; //таймфрейм индикатора iRSISign
uint ATR_Period=14;
...
int OnInit()
  {
//---- получение хендла индикатора iRSISign
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"iRSISign",ATR_Period,RSIPeriod,RSIPrice,UpLevel,DnLevel);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора iRSISign");
      return(INIT_FAILED);
     }
