...
//+----------------------------------------------+
//| Входные параметры индикатора AroonHornSign   |
//+----------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H4; //таймфрейм индикатора AroonHornSign
...
int OnInit()
  {
//---- получение хендла индикатора AroonHornSign
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"AroonHornSign",AroonPeriod,0);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора AroonHornSign");
      return(INIT_FAILED);
     }
