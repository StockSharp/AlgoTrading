...
//+----------------------------------------------+
//| Входные параметры индикатора WPRSIsignal     |
//+----------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H1; //таймфрейм индикатора WPRSIsignal
...
int OnInit()
  {
//---- получение хендла индикатора WPRSIsignal
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"WPRSIsignal",WPRSI_period,filterUP,filterDN);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора WPRSIsignal");
      return(INIT_FAILED);
     }
//---- инициализация переменной для хранения периода графика в секундах  
   TimeShiftSec=PeriodSeconds(InpInd_Timeframe);
//---- инициализация переменных начала отсчета данных
   int FilterMax=int(2+MathMax(filterUP,filterDN));
   min_rates_total=int(WPRSI_period+FilterMax+SignalBar);
