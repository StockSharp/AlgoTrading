...
//+----------------------------------------------+
//| Входные параметры индикатора ColorHMA        |
//+----------------------------------------------+
...
int OnInit()
  {
//---- получение хендла индикатора ColorHMA
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"ColorHMA",HMA_Period,0);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора ColorHMA");
      return(INIT_FAILED);
     }
//---- инициализация переменной для хранения периода графика в секундах  
   TimeShiftSec=PeriodSeconds(InpInd_Timeframe);
//---- инициализация переменных начала отсчета данных
   int Sqrt_Period=int(MathFloor(MathSqrt(HMA_Period)));
//---- инициализация переменных начала отсчета данных
