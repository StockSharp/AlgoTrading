...
//+----------------------------------------------+
//| ¬ходные параметры индикатора SuperWoodiesCCI |
//+----------------------------------------------+
...
int OnInit()
  {
//---- getting the SuperWoodiesCCI indicator handle
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"SuperWoodiesCCI",CCI_Period,TCCI_Period,applied_price,0);
   if(InpInd_Handle==INVALID_HANDLE) Print(" Failed to get handle of SuperWoodiesCCI indicator");
   ...
   if(InpInd_Timeframe==Period())
     {
      ...
      //---- add the SuperWoodiesCCI indicator on the chart  
      if(!cchart.IndicatorAdd(1,InpInd_Handle)) Print(" Failed to add the SuperWoodiesCCI indicator on the chart!");
     }
...
//---- delete the indicator from the chart
   string short_name="SuperWoodiesCCI";
   if(InpInd_Timeframe==Period())
     {
