...
//+--------------------------------------------------+
//| Входные параметры для индикатора Laguerre_ROC    |
//+--------------------------------------------------+
...
int OnInit()
  {
//---- получение хендла индикатора Laguerre_ROC
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"Laguerre_ROC",vPeriod,gamma,UpLevel,DnLevel,clrTeal,clrMagenta,1,1,0);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print("Не удалось получить хендл индикатора Laguerre_ROC");
      return(INIT_FAILED);
     }
   ...
   if(!SignalBar || NB.IsNewBar(Symbol(),InpInd_Timeframe) || Recount) // проверка на появление нового бара
     {
      ...
      if(SellPosClose && Col[1]>2) SELL_Close=true;
