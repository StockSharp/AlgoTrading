...
#property description "VininI_Trend_LRMA"
...
#property indicator_color1  clrDarkOrange,clrPurple,clrGray,clrSteelBlue,clrLime
//---- Indicator line width is equal to 2
#property indicator_width1  2
//---- displaying the indicator label
#property indicator_label1  "VininI_Trend_LRMA"
//+-----------------------------------+
...
CXMA XMA1[],XMA2;
//---- declaration of variable of the class CLRMA from the file IndicatorsAlgorithms.mqh
CLRMA LRMA1;
//+-----------------------------------+
...
input uint MA_Count=50;  //number of smoothing steps
...
input int UpLevel=+50; //Up level (range 0/+100)
input int DnLevel=-50; //Down level (range -100/0)
...
int min_rates_total,min_rates_1,min_rates_2,nsize,period[];
//+------------------------------------------------------------------+   
...
//---- Initialization of variables of the start of data calculation
   min_rates_1=int(LRMAPeriod); 
   min_rates_2=min_rates_1+XMA2.GetStartBars(MA_Method1,Length1+MA_Step*(MA_Count-1),Phase1);
   min_rates_total=min_rates_2+XMA2.GetStartBars(MA_Method2,Length2,Phase2);
...
//--- creation of the name to be displayed in a separate sub-window and in a pop up help
   IndicatorSetString(INDICATOR_SHORTNAME,"VininI_Trend_LRMA");
...
//---- declaration of variables with a floating point  
   double price_,lrma,xlrma,res,xres;
   ...
   for(bar=first; bar<rates_total && !IsStopped(); bar++)
     {
      ...
      lrma=LRMA1.LRMASeries(0,prev_calculated,rates_total,LRMAPeriod,price_,bar,false);
      sum=0;

      for(int sm=0; sm<nsize; sm++)
        {
         xlrma=XMA1[sm].XMASeries(min_rates_1,prev_calculated,rates_total,MA_Method1,Phase1,period[sm],lrma,bar,false);
         if(lrma>xlrma) sum++;
         else sum--;
        }

      res=100*sum/nsize;
      xres=XMA2.XMASeries(min_rates_2,prev_calculated,rates_total,MA_Method2,Phase2,Length2,res,bar,false);
      IndBuffer[bar]=xres;
