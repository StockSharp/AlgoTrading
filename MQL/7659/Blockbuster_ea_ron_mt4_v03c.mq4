...
int init()
  {
   if(LotIncrease)
     {
      StartingBalance=AccountBalance()/Lots;logwrite(TradeComment,"LotIncrease ACTIVE Account balance="+AccountBalance()+" Lots="+Lots+" StartingBalance="+StartingBalance);
     }
   else
     {
      ...
      while(true)
        {
         ...
         //
         ticket=OrderSend(Symbol(),OP_BUY,Lots,Ask,Slippage,SL,TP,TradeComment,MagicNumber,Blue);
         gle=GetLastError();
         if(gle==0)
           {
...
void logwrite (string filename, string mydata)
  {
   int myhandle;
   myhandle=FileOpen(Symbol()+"_"+filename,
