...
/*
   ...
   v6
      ...
      Добавлена возможность дополнительных проверок при закрытии ордеров по индикатору: проверка 
      прибыли ордера (закрытие только ордеров с прибылью не меньше заданной), проверка положения 
      стоплосс ордера (если стоплосс фиксирует заданную прибыль, ордер не закрывается).
      ...
      например, канал - http://codebase.mql4.com/ru/546, двухцветный попеременный - http://codebase.mql4.com/ru/688,
      рисущий точки или стрелки, при этом модификация ордера будлет происходить при наличие точки или стрелки.
...
/*
   ...
   v5
      ...
      с int на double, для возможности вводить дробные числа.  
...
extern string  s0="==== Индикатор для открытия (Mode: 1 - стрелки, 2 - главная и сигнальная, 3 - линия и уровни, 4 - экстремум) ====";
...
extern double  _O_M3_BuyLevel=20; // уровень попкупки (пересечение снизу вверх)
...
extern string  s2="==== Индикатор для закрытия (Mode: 1 - стрелки, 2 - главная и сигнальная, 3 линия и уровни, 4 экстремум) ====";
...
extern double  _C_M3_CloseBuyLevel=80; // уровень закрытия попкупки (пересечение сверху вниз)
...
extern int     PendLevel=20; // уровень установки отложенного ордера от текущй рыночной цены
...
extern bool    PendPriceFollow=false; // режим следования за ценой. ордер модифицируется при каждом изменении цены определнной переменной PendPromPrice, ордер переустанавливается только на "лучшую цену" (см. описание переменной PendNewSigMode)
...
extern int     _TS_Indent=0; // отступ в пунктах от значения индикатора
extern int     _TS_TrailInProfit=0; // минимальная фиксируемая прибыль. Стоплосс переставляется в том случае, если он фиксирует не менее чем _TS_TrailInProfit пунктов прибыли 
