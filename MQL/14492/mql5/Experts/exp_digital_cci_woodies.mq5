...
int OnInit()
  {
//---- получение хендла индикатора Digital_CCI_Woodies
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"Digital_CCI_Woodies",MA_Method1,Length1,Phase1,MA_Method2,Length2,Phase2,IPC,0);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора Digital_CCI_Woodies");
      return(INIT_FAILED);
     }
...
//---- инициализация переменных начала отсчета данных
   int min_rates_=26;
   int min_rates_1=min_rates_+GetStartBars(MA_Method1,Length1,Phase1);
   int min_rates_2=min_rates_+GetStartBars(MA_Method2,Length2,Phase2);
   ...
   if(!SignalBar || NB.IsNewBar(Symbol(),InpInd_Timeframe) || Recount) // проверка на появление нового бара
     {
      ...
      if(UpValue[1]>DnValue[1])
        {
         if(BuyPosOpen && UpValue[0]<=DnValue[0]) BUY_Open=true;
      ...
      if(UpValue[1]<DnValue[1])
        {
         if(SellPosOpen && UpValue[0]>=DnValue[0]) SELL_Open=true;
