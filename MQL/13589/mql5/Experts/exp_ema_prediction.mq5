...
//+----------------------------------------------+
//| Входные параметры индикатора EMA_Prediction  |
//+----------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H6; // Таймфрейм индикатора EMA_Prediction
...
int OnInit()
  {
//---- получение хендла индикатора EMA_Prediction
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"EMA_Prediction",FastMAPeriod,FastMAType,FastMAPrice,SlowMAPeriod,SlowMAType,SlowMAPrice);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора EMA_Prediction");
      return(INIT_FAILED);
     }
