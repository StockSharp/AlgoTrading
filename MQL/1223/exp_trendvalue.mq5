...
//+----------------------------------------------+
//| ¬ходные параметры индикатора TrendValue      |
//+----------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H4; //indicator time frame
input int    MAperiod=13;           // Moving averages period
...
int OnInit()
  {
//---- getting the TrendValue indicator handle
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"TrendValue",MAperiod,shiftPercent,ATRPeriod,ATRSensitivity);
   if(InpInd_Handle==INVALID_HANDLE) Print(" Failed to get handle of TrendValue indicator");

//---- initialization of a variable for storing a chart period in seconds  
   TimeShiftSec=PeriodSeconds(InpInd_Timeframe);

//---- initialization of variables of the start of data calculation
   min_rates_total=int(ATRPeriod+MAperiod);
