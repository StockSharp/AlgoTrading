...
//+-------------------------------------------------+
//| Входные параметры индикатора ForecastOscilator  |
//+-------------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H12; // Таймфрейм индикатора ForecastOscilator
...
int OnInit()
  {
//---- получение хендла индикатора ForecastOscilator
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"ForecastOscilator",length,t3,b,IPC);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора ForecastOscilator");
      return(INIT_FAILED);
     }
