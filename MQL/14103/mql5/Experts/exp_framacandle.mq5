...
//+------------------------------------------------+
//| Входные параметры индикатора FrAMACandle       |
//+------------------------------------------------+
...
int OnInit()
  {
//---- получение хендла индикатора FrAMACandle
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"FrAMACandle",FrAMA_Period);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора FrAMACandle");
      return(INIT_FAILED);
     }
   ...
   if(!SignalBar || NB.IsNewBar(Symbol(),InpInd_Timeframe) || Recount) // проверка на появление нового бара
     {
      ...
      if(TrendValue[1]==2)
        {
         if(BuyPosOpen && TrendValue[0]<2) BUY_Open=true;
      ...
      if(TrendValue[1]==0)
        {
         if(SellPosOpen && TrendValue[0]>0) SELL_Open=true;
