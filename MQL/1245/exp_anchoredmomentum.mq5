...
//+----------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H4; //timeframe of the AnchoredMomentum indicator
...
int OnInit()
  {
//---- getting the AnchoredMomentum indicator handle
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"AnchoredMomentum",MomPeriod,SmoothPeriod,IPC,UpLevel,DnLevel,0);
   if(InpInd_Handle==INVALID_HANDLE) Print(" Failed to get handle of AnchoredMomentum indicator");
   ...
   if(!SignalBar || NB.IsNewBar(Symbol(),InpInd_Timeframe) || Recount) // checking for a new bar
     {
      ...
      if(Value[1]>UpLevel)
        {
         if(BuyPosOpen) if(Value[0]<=UpLevel) BUY_Open=true;
      ...
      if(Value[1]<DnLevel)
        {
         if(SellPosOpen) if(Value[0]<=DnLevel) SELL_Open=true;
