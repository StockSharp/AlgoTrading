...
int OnInit()
  {
//---- получение хендла индикатора RVIDiff
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"RVIDiff",RVIPeriod,XMA_Method,XLength,XPhase,0);
   if(InpInd_Handle==INVALID_HANDLE)  
     {
      Print(" Не удалось получить хендл индикатора RVIDiff");
      return(INIT_FAILED);
     }
...
//---- Инициализация переменных начала отсчёта данных
   min_rates_total=int(RVIPeriod)+1+3;
