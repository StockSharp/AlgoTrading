//+------------------------------------------------------------------+
//|                                Exp_Ergodic_Ticks_Volume_OSMA.mq5 |
...
int OnInit()
  {
//---- получение хендла индикатора Ergodic_Ticks_Volume_OSMA
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"Ergodic_Ticks_Volume_OSMA",VolumeType,XMA_Method,XLength1,XLength2,XLength3,XLength4,XLength5,XLength6,XLength7,XPhase,0);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора Ergodic_Ticks_Volume_OSMA");
      return(INIT_FAILED);
     }
...
//---- инициализация переменных начала отсчета данных
   ...
   min_rates_total+=GetStartBars(XMA_Method,XLength5,XPhase);
   min_rates_total+=GetStartBars(XMA_Method,XLength6,XPhase);
   min_rates_total+=GetStartBars(XMA_Method,XLength7,XPhase);
   ...
   if(!SignalBar || NB.IsNewBar(Symbol(),InpInd_Timeframe) || Recount) // проверка на появление нового бара
     {
      ...
      if(Ind[1]<Ind[2])
        {
         if(BuyPosOpen && Ind[0]>=Ind[1]) BUY_Open=true;
      ...
      if(Ind[1]>Ind[2])
        {
         if(SellPosOpen && Ind[0]<=Ind[1]) SELL_Open=true;
