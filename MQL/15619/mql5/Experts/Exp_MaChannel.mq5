...
//+------------------------------------------------+
//| Входные параметры индикатора MaChannel         |
//+------------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H1;     //таймфрейм индикатора MaChannel
...
int OnInit()
  {
//---- получение хендла индикатора MaChannel
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"MaChannel",XMA_Method,XLength,XPhase);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора MaChannel");
      return(INIT_FAILED);
     }
