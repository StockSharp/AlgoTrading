...
int OnInit()
  {
//---- получение хендла индикатора BnB
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"BnB",XMA_Method,XLength,XPhase,VolumeType);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора BnB");
      return(INIT_FAILED);
     }
   ...
   if(!SignalBar || NB.IsNewBar(Symbol(),InpInd_Timeframe) || Recount) // проверка на появление нового бара
     {
      ...
      if(CopyBuffer(InpInd_Handle,1,SignalBar,2,Ind)<=0) {Recount=true; return;}
      if(CopyBuffer(InpInd_Handle,0,SignalBar,2,Sign)<=0) {Recount=true; return;}
