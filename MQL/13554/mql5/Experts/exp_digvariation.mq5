...
//+----------------------------------------------+
//| Входные параметры индикатора DigVariation    |
//+----------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H8;    // Таймфрейм индикатора
input uint Period_=12;                               // Период усреднения
...
int OnInit()
  {
//---- получение хендла индикатора DigVariation
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"DigVariation",Period_,MA_Method_,SmoothPower,0);
   if(InpInd_Handle==INVALID_HANDLE)  
     {
      Print(" Не удалось получить хендл индикатора DigVariation");
      return(INIT_FAILED);
     }
