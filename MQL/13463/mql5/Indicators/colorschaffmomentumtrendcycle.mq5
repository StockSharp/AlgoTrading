//+------------------------------------------------------------------+
//|                                ColorSchaffMomentumTrendCycle.mq5 |
//|                                  Copyright © 2011, EarnForex.com |
//|                                        http://www.earnforex.com/ |
//+------------------------------------------------------------------+ 
#property copyright "Copyright © 2011, EarnForex.com"
#property link      "http://www.earnforex.com"
#property description "Schaff Trend Cycle - Cyclical Stoch over Stoch over Momentum MACD."
...
#property indicator_label1 "Schaff Momentum Trend Cycle"
//+-----------------------------------------------+
...
//+-----------------------------------------------+
input uint Fast_Momentum = 23; // Период быстрого Momentum
input uint Slow_Momentum = 50; // Период медленного Momentum
...
//---- инициализация переменных начала отсчета данных
   min_rates_1=int(Fast_Momentum);
   min_rates_2=int(Slow_Momentum);
   min_rates_total=MathMax(min_rates_1,min_rates_2)+int(Cycle)+1;
//--- получение хендла индикатора MA 1
   Ind1_Handle=iMomentum(Symbol(),PERIOD_CURRENT,Fast_Momentum,AppliedPrice);
   if(Ind1_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора Momentum 1");
      return(INIT_FAILED);
     }
//--- получение хендла индикатора MA 2
   Ind2_Handle=iMomentum(Symbol(),PERIOD_CURRENT,Slow_Momentum,AppliedPrice);
   if(Ind2_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора Momentum 2");
      return(INIT_FAILED);
     }
...
//---- инициализация переменной для короткого имени индикатора
   string shortname;
   StringConcatenate(shortname,"Schaff Momentum Trend Cycle( ",Fast_Momentum,", ",Slow_Momentum,", ",Cycle," )");
...
//---- объявление переменных с плавающей точкой  
   double fastMomentum[],slowMomentum[],LLV,HHV;
...
//---- копируем вновь появившиеся данные в массивы
   if(CopyBuffer(Ind1_Handle,0,0,to_copy,fastMomentum)<=0) return(RESET);
   if(CopyBuffer(Ind2_Handle,0,0,to_copy,slowMomentum)<=0) return(RESET);
//---- индексация элементов в массивах как в таймсериях  
   ArraySetAsSeries(fastMomentum,true);
   ArraySetAsSeries(slowMomentum,true);
   ...
   for(bar=limit; bar>=0 && !IsStopped(); bar--)
     {
      ...
      Bar1=Count[1];
      //----
      MACD[Bar0]=fastMomentum[bar]-slowMomentum[bar];
      //----
