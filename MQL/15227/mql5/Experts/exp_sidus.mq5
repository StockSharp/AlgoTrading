...
//+----------------------------------------------+
//| Входные параметры индикатора Sidus           |
//+----------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H1; //таймфрейм индикатора Sidus
input int FastEMA=18;                             //Период быстрой EMA
input int SlowEMA=28;                             //Период медленной EMA
input int FastLWMA=5;                             //Период быстрой LWMA
input int SlowLWMA=8;                             //Период медленной LWMA
input ENUM_APPLIED_PRICE IPC=PRICE_CLOSE;         //Ценовая константа
input uint Digit=0;                               //Размах в пунктах
...
int OnInit()
  {
//---- получение хендла индикатора Sidus
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"Sidus",FastEMA,SlowEMA,FastLWMA,SlowLWMA,IPC,Digit);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора Sidus");
      return(INIT_FAILED);
     }
//---- инициализация переменной для хранения периода графика в секундах  
   TimeShiftSec=PeriodSeconds(InpInd_Timeframe);
//---- инициализация переменных начала отсчета данных
   min_rates_total=int(MathMax(FastLWMA,SlowLWMA)+3+SignalBar+3);
//----
   return(0);
  }
//+------------------------------------------------------------------+
