...
extern double CertaintyBars = 0.3;
...
bool bigBoyBullish = false;
bool bigBoyBearish = false;
...
void managePositions(){
   if( ( totalHistoryProfit < 0 || totalTrades == 1 ) && MathAbs( totalHistoryProfit ) < totalProfit * BasketProfit  ) closeAll( "profits" );
      ...
      for( int i = 0 ; i < OrdersTotal() ; i++ ) {
         ...
         if( OrderSymbol() == Symbol() && OrderComment() == "bigboy" ) { 
            if( Bid > OrderOpenPrice() && fractalDownPrice > OrderStopLoss() ){
...
void openPosition(){ 
   ...
   if( !nearLongPosition && certaintyBullish ) OrderSend( Symbol(), type, lotSize, Ask, slippage, 0, 0, "certanty", MAGIC ) ;
   else if( !nearShortPosition && certaintyBearish ) OrderSend( Symbol(), type , lotSize, Bid, slippage, 0, 0, "certanty", MAGIC ) ;
 
   if( !nearLongPosition && shortTrendUp && bigBoyBullish ) OrderSend( Symbol(), type, lotSize, Ask, slippage, 0, 0, "certanty", MAGIC ) ;
   else if( !nearShortPosition && shortTrendDown && bigBoyBearish ) OrderSend( Symbol(), type , lotSize, Bid, slippage, 0, 0, "certanty", MAGIC ) ;
