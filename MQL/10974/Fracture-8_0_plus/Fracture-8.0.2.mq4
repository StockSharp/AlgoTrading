...
void openPosition(){ 
   ...
   if( Certanty99 && trendStarting ){ 
      if( !nearLongPosition && shortTrendUp ) OrderSend( Symbol(), type, lotSize, Ask, slippage, 0, 0, "certanty", MAGIC ) ;
      if( !nearShortPosition && shortTrendDown ) OrderSend( Symbol(), type , lotSize, Bid, slippage, 0, 0, "certanty", MAGIC ) ;
   }
   if( BigBoy && bigBoy ){ 
      if( !nearLongPosition && shortTrendUp ) OrderSend( Symbol(), type, lotSize, Ask, slippage, 0, 0, "certanty", MAGIC ) ;
      if( !nearShortPosition && shortTrendDown ) OrderSend( Symbol(), type , lotSize, Bid, slippage, 0, 0, "certanty", MAGIC ) ;
...
int trendingCertainty(){ 
   ...
   if( totalUp > CertaintyBars && Close[0] > Open[0] ) return ( OP_BUY );
   else if( totalDown > CertaintyBars && Close[0] < Open[0] ) return ( OP_SELL ); 
}
