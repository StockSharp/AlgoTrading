...
int prepareCertainty(){ 
   ...
   if( totalUp > CertaintyCount && Close[0] > Open[CertaintyBars] && Close[0] > Open[0] && Close[0] < fractalUpPrice && fractalUpPrice != 0 ) certaintyBullish = true;
   else certaintyBullish = false; 
   if( totalDown > CertaintyCount && Close[0] < Open[CertaintyBars] && Close[0] < Open[0] && Close[0] > fractalDownPrice && fractalUpPrice != 0 ) certaintyBearish = true;
   else certaintyBearish = false;
} 
/*
void prepareBreakout(){
   bool BullishCross1 = false;
   bool BullishCross2 = false;  
   bool BearishCross1 = false;
   bool BearishCross2 = false; 
   for( int i = 0; i < CrossBars; i++ ){ 
      ...
      if( tMA1Cur > tMA2Cur && tMA1Prev <= tMA2Prev ) BearishCross1 = true;
      if( tMA2Cur < tMA3Cur && tMA2Prev >= tMA3Prev ) BearishCross2 = true; 
      if( tMA1Cur < tMA2Cur && tMA1Prev >= tMA2Prev ) BullishCross1 = true;
      if( tMA2Cur > tMA3Cur && tMA2Prev <= tMA3Prev ) BullishCross2 = true;  
      if( ( shortBullishCross1 && shortBullishCross2 ) || ( shortBearishCross1 && shortBearishCross2 ) ) break;
   }
   if( ( shortBullishCross1 && shortBullishCross2 ) ){
      int bars = 0; 
      double averageATR = 0;
      double totalATR = 0;
      double AnticipationBarsDouble = ( double ) AnticipationBars;
      for( i = 0; i < AnticipationBars; i++ ) totalATR = totalATR + iATR( NULL, ATRTimeFrame, ATRPeriod, i );
      averageATR = averageATR / AnticipationBarsDouble; 
      int anticipationADXCount = 0;
      int anticipationATRCount = 0;
      for( i = 0; i < AnticipationBars; i++ ){
      ...
      if( anticipationATRCount > AnticipationATRCount && anticipationADXCount > AnticipationADXCount ) breakoutBullish = true;
      else breakoutBullish = false;
   } else if(( shortBearishCross1 && shortBearishCross2 )){
      bars = 0; 
      averageATR = 0;
      totalATR = 0;
      double AnticipationBarsDouble1 = ( double ) AnticipationBars;
      for( i = 0; i < AnticipationBars; i++ ) totalATR = totalATR + iATR( NULL, ATRTimeFrame, ATRPeriod, i );
      averageATR = averageATR / AnticipationBarsDouble1; 
      anticipationADXCount = 0;
      anticipationATRCount = 0;
      for( i = 0; i < AnticipationBars; i++ ){
         tATR = iATR( NULL, ATRTimeFrame, ATRPeriod, i );
         tADXMainCur = iADX( NULL, ATRTimeFrame, ATRPeriod, PRICE_HIGH, MODE_MAIN, i );
         tADXPlusCur = iADX( NULL, ATRTimeFrame, ATRPeriod, PRICE_HIGH, MODE_PLUSDI, i );
         tADXMinusCur = iADX( NULL, ATRTimeFrame, ATRPeriod, PRICE_HIGH, MODE_MINUSDI, i );
         tADXMainPrev = iADX( NULL, ATRTimeFrame, ATRPeriod, PRICE_HIGH, MODE_MAIN, i + 1 );
         tADXPlusPrev = iADX( NULL, ATRTimeFrame, ATRPeriod, PRICE_HIGH, MODE_PLUSDI, i + 1 );
         tADXMinusPrev = iADX( NULL, ATRTimeFrame, ATRPeriod, PRICE_HIGH, MODE_MINUSDI, i + 1 ); 
         if( tADXPlusCur > tADXMinusCur && tADXPlusPrev <= tADXMinusPrev || tADXPlusCur < tADXMinusCur && tADXPlusPrev >= tADXMinusPrev ) 
            anticipationADXCount = anticipationADXCount + 1; 
         if( tATR < eATR * AnticipationATR ) anticipationATRCount = anticipationATRCount + 1; 
      }
      if( anticipationATRCount > AnticipationATRCount && anticipationADXCount > AnticipationADXCount ) breakoutBearish = true;
      else breakoutBearish = false;
   }
...
void openPosition(){ 
   ...
   RefreshRates(); 
   if( rangingMarket ){  
      if( !nearLongPosition && Close[0] <= fractalDownPrice && Close[0] >= MA1Cur ) OrderSend( Symbol(), OP_BUY, lotSize, Ask, slippage, 0, 0, "default", MAGIC );
      if( !nearShortPosition && Close[0] >= fractalUpPrice && Close[0] <= MA1Cur ) OrderSend( Symbol(), OP_SELL, lotSize, Bid, slippage, 0, 0, "default", MAGIC );
   }
   if( CertaintySystem ){
      if( !nearLongPosition && certaintyBullish ) OrderSend( Symbol(), OP_BUY, lotSize, Ask, slippage, 0, 0, "certanty", MAGIC );
      else if( !nearShortPosition && certaintyBearish ) OrderSend( Symbol(), OP_SELL, lotSize, Bid, slippage, 0, 0, "certanty", MAGIC );
   }
   if( BreakoutSystem ){
      if( !nearLongPosition && shortTrendUp && breakoutBullish ) OrderSend( Symbol(), OP_BUY, lotSize, Ask, slippage, 0, 0, "certanty", MAGIC );
      else if( !nearShortPosition && shortTrendDown && breakoutBearish ) OrderSend( Symbol(), OP_SELL, lotSize, Bid, slippage, 0, 0, "certanty", MAGIC );
   }
}
