...
void openPosition(){ 
   ...
   if( totalTrades <= StartTrades || drawdown > TurboStart ){
      if( !nearLongPosition && totalTrades == 0 && type == -1 && ( ( eADX > ADXLine && Close[0] > Open[0] ) || ( eADX < ADXLine && Close[0] < Open[0] ) ) ) OrderSend( Symbol(), OP_BUY, lotSize, Ask, slippage, 0, 0, "default", MAGIC );
      else if( !nearShortPosition && totalTrades == 0 && type == -1 && ( ( eADX > ADXLine && Close[0] < Open[0] ) ) || ( eADX < ADXLine && Close[0] > Open[0] ) ) OrderSend( Symbol(), OP_SELL, lotSize, Bid, slippage, 0, 0, "default", MAGIC );
   } else {
      if( !nearLongPosition && totalTrades == 0 && type == -1 && ( ( eADX > ADXLine && Close[0] < Open[0] ) || ( eADX < ADXLine && Close[0] < Open[0] ) ) ) OrderSend( Symbol(), OP_BUY, lotSize, Ask, slippage, 0, 0, "default", MAGIC );
      else if( !nearShortPosition && totalTrades == 0 && type == -1 && ( ( eADX > ADXLine && Close[0] > Open[0] ) ) || ( eADX < ADXLine && Close[0] > Open[0] ) ) OrderSend( Symbol(), OP_SELL, lotSize, Bid, slippage, 0, 0, "default", MAGIC );
   }   
...
int firstIn( int type ){ 
   ...
   for( int i = 0 ; i < OrdersTotal() ; i++ ) {
      ...
      if( OrderSymbol() == Symbol() && OrderType() == type ) {
         ...
         if( tradeTime > firstInTime ) {
            firstInTime = tradeTime;
            firstInLots = OrderLots(); 
            firstInTicket = OrderTicket(); 
         }
      }
   }
   return firstInTicket;
}

void OrderCloseFIFO( int ticket, double lots, double price, double slippage ){
   int firstIn = firstIn( int type ) ;
   while( ticketIn != 0 ){
      firstInTicket = firstIn();
      firstInLots = firstIn();
      if( firstIn != 0 && firstInLots < lots ){
         ticketIn =
         OrderClose( OrderTicket(), OrderLots(), Bid, slippage );
      } else {
         oppLots = 
         OrderSend( Symbol(), type , lotSize, Ask, slippage, 0, 0, "scalp", MAGIC );
      } 
   }
}

void OrderSendFIFO( int symbol, int type, double lots, double price, double slippage, double stopLoss, double takeProfit, string comment, string magic ){
