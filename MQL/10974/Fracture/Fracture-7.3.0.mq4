...
extern double TurboProfit = 1.35;
...
void lotSize(){ 
   ...
   if( rangingMarket && eADX < ADXLine1  ) lotSize = NormalizeDouble( ( AccountFreeMargin() * RangeUsage / marginRequirement ) * BaseLotSize , 2 ) ;
...
void managePositions(){
   ...
   else if( totalTrades > 1 && ( ( totalProfit > MathAbs( totalLoss ) * OpenProfit && drawdown < TurboStart ) || ( totalProfit > MathAbs( totalLoss ) * OpenProfit * TurboProfit && drawdown >= TurboStart ) ) ) closeAll();
   else {
      for( int i = 0 ; i < OrdersTotal() ; i++ ) {
         if( OrderSelect( i, SELECT_BY_POS, MODE_TRADES ) == false ) break;  
         if( OrderSymbol() == Symbol() && OrderComment() == "scalp" && drawdown > TurboStart ) { 
