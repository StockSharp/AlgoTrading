...
bool shortBullishCross = false;
bool shortBearishCross = false;
bool longBullishCross = false;
bool longBearishCross = false;
string display = "\n"; 
...
void openPosition(){ 
   ...
   if( rangingMarket && eADX < ADXLine ){ 
      ...
      if( shortBullishCross && longBullishCross  ) 
         if( !nearLongPosition  ) 
            OrderSend( Symbol(), OP_BUY , lotSize, Ask, slippage, 0, 0, "scalp", MAGIC ) ;
      if( shortBearishCross && longBearishCross   ) 
        if( !nearShortPosition ) 
...
void prepareTrend(){ 
   shortBullishCross = false;
   shortBearishCross = false;
   longBullishCross = false;
   bool longBearishCross = false;
   for( int i = 0; i < 30; i++ ){ 
      ...
      if( tMA1Cur > tMA2Cur && tMA1Prev <= tMA2Prev ) shortBullishCross = true;
      if( tMA1Cur < tMA2Cur && tMA1Prev >= tMA2Prev ) shortBearishCross = true; 
      if( tMA2Cur > tMA3Cur && tMA2Prev <= tMA3Prev ) longBullishCross = true; 
      if( tMA2Cur < tMA3Cur && tMA2Prev >= tMA3Prev ) longBearishCross = true; 
   }
