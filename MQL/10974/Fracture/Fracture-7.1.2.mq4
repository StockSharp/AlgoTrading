...
extern int MaxScalping = 7; 
extern int OpenScalping = 5;
...
void update(){
   ...
   display = display + "\n totalScalping: " + DoubleToStr( totalScalping, 0 );
   Comment( display );
}
...
void openPosition(){ 
   ...
   if( rangingMarket && eADX < ADXLine ){ 
      ...
      if( !nearLongPosition && shortBullishCross1 && shortBullishCross2 && totalHistoryScalpingBuys < MaxScalping && totalOpenScalpingBuys < OpenScalping )  
         OrderSend( Symbol(), OP_BUY , lotSize, Ask, slippage, 0, 0, "scalp", MAGIC ) ;  
      if( !nearShortPosition && shortBearishCross1 && shortBearishCross2 && totalHistoryScalpingSells < MaxScalping && totalOpenScalpingSells < OpenScalping )  
...
void prepareTrend(){ 
   ...
   for( int i = 0; i < 30; i++ ){ 
      ...
      if( tMA1Cur > tMA2Cur && tMA1Prev <= tMA2Prev ) shortBullishCross1 = true;
      if( tMA1Cur < tMA2Cur && tMA1Prev >= tMA2Prev ) shortBullishCross2 = true;   
      if( tMA1Cur < tMA2Cur && tMA1Prev >= tMA2Prev ) shortBearishCross1 = true;
      if( tMA1Cur > tMA2Cur && tMA1Prev <= tMA2Prev ) shortBearishCross2 = true;  
   }
   ...
   if( ( totalHistoryProfit < 0 || totalTrades == 1 ) && MathAbs( totalHistoryProfit ) < totalProfit * BasketProfit  ) closeAll( "profits" );
      ...
      for( int i = 0 ; i < OrdersTotal() ; i++ ) {
         ...
         if( totalTrades < MaxTrades -1 ){ 
            ...
            if( OrderSymbol() == Symbol() && OrderComment() == "scalp" ) { 
               if( OrderType() == OP_BUY && Bid > OrderOpenPrice() && Bid >= fractalUpPrice ) 
                  OrderClose( OrderTicket(), OrderLots(), Bid, slippage ); 
               else if( OrderType() == OP_SELL && Ask < OrderOpenPrice() && Ask <= fractalDownPrice )
                  OrderClose( OrderTicket(), OrderLots(), Ask, slippage );  
            }
         } 
