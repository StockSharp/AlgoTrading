...
extern int StartTrades = 4;
extern double BasketProfit = 3;
extern double OpenProfit = 1.2; 
extern double MinProfit = 0.5;
extern double TradeSpace = 1.0;
extern double RangingMarket = 0.5;
extern double Aggressive = 8;
extern double DynamicSlippage = 1;
extern double TurboStart = 0.10;
extern double TrendReversal = 0.20;
extern string RISK = ".............................................................................................................";
extern int MaxTrades = 7;
extern double BaseLotSize = 0.01;
extern double RangeUsage = 0.08;
extern double TrendUsage = 0.05;
...
int firstInLongTicket = 0;
int firstInLongTime = 0; 
int firstInShortTicket = 0;
int firstInShortTime = 0;

double firstInLongLots = 0;
double firstInShortLots = 0;

double combinedProfitLoss = 0;
...
void lotSize(){ 
   ...
   drawdown = 1 - ( AccountBalance() - combinedProfitLoss ) / AccountBalance();
...
void preparePositions() {
   ...
   for( int i = 0 ; i < OrdersTotal() ; i++ ) {
      ...
      if( OrderSymbol() == Symbol() && OrderStopLoss() == 0 ) {
         ...
         combinedProfitLoss = combinedProfitLoss + OrderProfit();
      }
   }
}
...
void openPosition(){ 
   ...
   if( totalTrades < 1 - ( StartTrades / MaxTrades ) || totalTrades == 0 ){
      if( drawdown < TrendReversal ){
         if( !nearLongPosition && totalTrades == 0 && type == -1 && ( ( eADX > ADXLine && Close[0] < Open[0] && longTrendUp ) || ( eADX < ADXLine && Close[0] < Open[0] && shortTrendDown ) ) ) OrderSend( Symbol(), OP_BUY, lotSize, Ask, slippage, 0, 0, "scalp", MAGIC );
         else if( !nearShortPosition && totalTrades == 0 && type == -1 && ( ( eADX > ADXLine && Close[0] > Open[0] && longTrendDown ) || ( eADX < ADXLine && Close[0] > Open[0] && shortTrendUp ) ) ) OrderSend( Symbol(), OP_SELL, lotSize, Bid, slippage, 0, 0, "scalp", MAGIC );
      } else {
         if( !nearLongPosition && totalTrades == 0 && type == -1 && ( ( eADX > ADXLine && Close[0] > Open[0] && longTrendUp ) || ( eADX < ADXLine && Close[0] < Open[0]  && shortTrendDown ) ) ) OrderSend( Symbol(), OP_BUY, lotSize, Ask, slippage, 0, 0, "default", MAGIC );
         else if( !nearShortPosition && totalTrades == 0 && type == -1 && ( ( eADX > ADXLine && Close[0] < Open[0] && longTrendDown ) || ( eADX < ADXLine && Close[0] > Open[0] && shortTrendUp ) ) ) OrderSend( Symbol(), OP_SELL, lotSize, Bid, slippage, 0, 0, "default", MAGIC );
      }
   }
}
...
void prepareFirstIn(){
   ...
   for( int i = 0 ; i < OrdersTotal() ; i++ ) {
      ...
      if( OrderType() == OP_BUY ){
         if( tradeTime > firstInLongTime ){
            firstInLongTime = tradeTime;
            firstInLongTicket = OrderTicket();
            firstInLongLots = OrderLots();
         }
      } else if( OrderType() == OP_SELL ){ 
         if( tradeTime > firstInShortTime ){
            firstInShortTime = tradeTime;
            firstInShortTicket = OrderTicket();
            firstInShortLots = OrderLots();
         }
      }
   }
}
*/
/*
int OrderSendFIFO( int symbolFIFO, int typeFIFO, double lotsFIFO, double priceFIFO, double slippageFIFO, double stopLossFIFO, double takeProfitFIFO, string commentFIFO, int magicFIFO ){
   int fifo = -1;
   prepareFirstIn();
   if( typeFIFO == OP_BUY ){
      fifo = -1;
      while( firstInShortTicket != 0 ){
         RefreshRates();
         if( lotsFIFO <= firstInShortLots ) fifo = OrderClose( firstInShortTicket, firstInShortLots, Bid, slippage ); 
         else fifo = OrderSend( Symbol(), OP_BUY, MathAbs( lotsFIFO - firstInShortLots ), Ask, slippage, 0, 0, "scalp", MAGIC ) ;  
         prepareFirstIn();
      }
      if( fifo == -1 ) OrderSend( Symbol(), OP_BUY, lotsFIFO, Ask, slippage, 0, 0, "scalp", MAGIC );
   } else if( typeFIFO == OP_SELL ){
      fifo = -1;
      while( firstInLongTicket != 0 ){
         RefreshRates();
         if( lotsFIFO <= firstInLongLots ) fifo = OrderClose( firstInLongTicket, firstInLongLots, Ask, slippage ); 
         else fifo = OrderSend( Symbol(), OP_SELL, MathAbs( lotsFIFO - firstInLongLots ), Bid, slippage, 0, 0, "scalp", MAGIC ) ;  
         prepareFirstIn();
      }
      if( fifo == -1 ) OrderSend( Symbol(), OP_SELL, lotsFIFO, Bid, slippage, 0, 0, "scalp", MAGIC ) ;
   }
}
*/
/*
int OrderCloseFIFO( int ticketFIFO, double lotsFIFO, double priceFIFO, double slippageFIFO ){
   int fifo = -1;
   prepareFirstIn();  
   OrderSelect( ticketFIFO, SELECT_BY_TICKET );
   if( OrderType() == OP_BUY ){
      fifo = -1;
      while( firstInShortTicket != 0 ){
         if( lotsFIFO <= firstInShortLots ) fifo = OrderClose( firstInShortTicket, firstInShortLots, Bid, slippage ); 
         else fifo = OrderSend( Symbol(), OP_BUY, MathAbs( lotsFIFO - firstInShortLots ), Ask, slippage, 0, 0, "fifo close", MAGIC ) ;  
         prepareFirstIn();
      }
      if( fifo == -1 ) OrderSend( Symbol(), OP_BUY, lotsFIFO, Ask, slippage, 0, 0, "scalp", MAGIC );
   } else if( OrderType() == OP_SELL ){
      ...
      while( firstInLongTicket != 0 ){ 
         if( lotsFIFO <= firstInLongLots ) fifo = OrderClose( firstInLongTicket, firstInLongLots, Ask, slippage ); 
         else fifo = OrderSend( Symbol(), OP_SELL, MathAbs( lotsFIFO - firstInLongLots ), Bid, slippage, 0, 0, "fifo close", MAGIC ) ;  
         prepareFirstIn();
      }
      if( fifo == -1 ) OrderSend( Symbol(), OP_SELL, lotsFIFO, Bid, slippage, 0, 0, "scalp", MAGIC ) ;
   } 
