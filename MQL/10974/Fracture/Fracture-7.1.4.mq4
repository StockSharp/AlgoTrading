...
extern double TrendReversal = 0.05;
...
double slippage, marginRequirement, lotSize, lastProfit, totalHistoryProfit, totalProfit, totalLoss, symbolHistory, buyLots, sellLots,
...
void update(){
   ...
   display = display + "\n shortBullishCross1: " + DoubleToStr( shortBullishCross1, 0 );
   
   display = display + "\n shortBullishCross2: " + DoubleToStr( shortBullishCross2, 0 );
   
   display = display + "\n shortBearishCross1: " + DoubleToStr( shortBearishCross1, 0 );
   
   display = display + "\n shortBearishCross2: " + DoubleToStr( shortBearishCross2, 0 );
   Comment( display );
}
...
void openPosition(){ 
   ...
   if( totalScalping < MaxScalping || totalTrades < MaxScalping ){
      if( !nearLongPosition && shortBullishCross1 && shortBullishCross2 && totalOpenScalpingBuys < MaxScalping )  
         OrderSend( Symbol(), OP_BUY , lotSize, Ask, slippage, 0, 0, "scalp", MAGIC ) ;  
      if( !nearShortPosition && shortBearishCross1  && shortBearishCross1 && shortBearishCross2 && totalOpenScalpingSells < MaxScalping )  
...
void prepareTrend(){ 
   ...
   for( int i = 0; i < 30; i++ ){ 
      ...
      if( tMA1Cur > tMA2Cur && tMA1Prev <= tMA2Prev && Close[0] < MA2Cur  ) shortBearishCross1 = true;
      if( tMA2Cur < tMA3Cur && tMA2Prev >= tMA3Prev && Close[0] < MA2Cur  )  shortBearishCross2 = true; 
      if( tMA1Cur < tMA2Cur && tMA1Prev >= tMA2Prev && Close[0] > MA2Cur   )  shortBullishCross1 = true;
      if( tMA2Cur > tMA3Cur && tMA2Prev <= tMA3Prev && Close[0] > MA2Cur  ) shortBullishCross2 = true;  
...
//take profit to order modify
// on bullish cross close other.
// close all opem profit and take turbo profit