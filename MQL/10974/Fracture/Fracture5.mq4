...
void managePositions(){
   if( totalHistoryProfit < 0 && MathAbs( totalHistoryProfit ) < totalProfit * BasketProfit  ) closeAll( "profits" );
      ...
      for( int i = 0 ; i < OrdersTotal() ; i++ ) {
         if( OrderSelect( i, SELECT_BY_POS, MODE_TRADES ) == false ) break;
         if( OrderStopLoss() == 0 && totalTrades > 1 - MaxTrades / StopTrades ){
            if( OrderType() == OP_BUY ) {
               if( Bid > OrderOpenPrice() + ( StopTrailing + StartTrailing * eATR ) && fractalLowPriceFirst > OrderOpenPrice() && fractalLowPriceFirst - OrderOpenPrice() > ( StartTrailing + StopTrailing ) * eATR ) 
                  OrderModify( OrderTicket(), OrderOpenPrice(), fractalLowPriceFirst - ( StopTrailing * eATR ), OrderTakeProfit(), 0 );
             } else if( OrderType() == OP_SELL ) { 
               if( OrderStopLoss() == 0 && Ask < OrderOpenPrice() + ( StartTrailing + StopTrailing * eATR ) && fractalHighPriceFirst < OrderOpenPrice() && OrderOpenPrice() - fractalHighPriceFirst < ( StartTrailing + StopTrailing )  * eATR ) 
                  OrderModify( OrderTicket(), OrderOpenPrice(), fractalHighPriceFirst + ( StopTrailing * eATR ), OrderTakeProfit(), 0 ); 
             }
         } 
         if( OrderStopLoss() != 0 ){
            if( OrderType() == OP_BUY ) {
               if( fractalLowPriceFirst - OrderOpenPrice() > StopTrailing * eATR && OrderStopLoss() < fractalLowPriceFirst - ( StopTrailing * eATR ) ) 
                  OrderModify( OrderTicket(), OrderOpenPrice(), fractalLowPriceFirst - ( StopTrailing * eATR ), OrderTakeProfit(), 0 ); 
            } else if( OrderType() == OP_SELL ) { 
               if( OrderOpenPrice() - fractalHighPriceFirst > StopTrailing * eATR && OrderStopLoss() > fractalHighPriceFirst + ( StopTrailing * eATR ) ) 
                  OrderModify( OrderTicket(), OrderOpenPrice(), fractalHighPriceFirst + ( StopTrailing * eATR ), OrderTakeProfit(), 0 );
            }
         }
