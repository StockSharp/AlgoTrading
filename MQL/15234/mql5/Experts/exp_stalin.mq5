...
//+----------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H1; //таймфрейм индикатора Stalin
input ENUM_MA_METHOD MAMethod=MODE_EMA;           //метод усреднения мувинга
input int    MAShift=0;                           //сдвиг мувинга в барах
input int    Fast=14;                             //период быстрого мувинга
input int    Slow=21;                             //период медленного мувинга
input int    RSI=17;                              //период RSI
input int    Confirm=0;                           //уровень в пунктах
input int    Flat=0;                              //размах колебаний флета в пунктах
...
int OnInit()
  {
//---- получение хендла индикатора Stalin
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"Stalin",MAMethod,MAShift,Fast,Slow,RSI,Confirm,Flat,false,false);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора Stalin");
      return(INIT_FAILED);
     }
//---- инициализация переменной для хранения периода графика в секундах  
   TimeShiftSec=PeriodSeconds(InpInd_Timeframe);
//---- инициализация переменных начала отсчета данных
   min_rates_total=int(MathMax(RSI,MathMax(Slow,Fast))+SignalBar);
