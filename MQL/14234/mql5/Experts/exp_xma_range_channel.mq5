...
//+------------------------------------------------+
//| Входные параметры индикатора XMA_Range_Channel |
//+------------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H4; // Таймфрейм индикатора XMA_Range_Channel
...
int OnInit()
  {
//---- получение хендла индикатора XMA_Range_Channel
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"XMA_Range_Channel",XMA_Method,XLength,XPhase,0,0);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора XMA_Range_Channel");
      return(INIT_FAILED);
     }
   ...
   if(!SignalBar || NB.IsNewBar(Symbol(),InpInd_Timeframe) || Recount) // проверка на появление нового бара
     {
      ...
      if(CopyBuffer(InpInd_Handle,8,SignalBar,2,Clr)<=0) {Recount=true; return;}
      //---- получим сигналы для покупки
      if(Clr[1]>1 && Clr[1]!=EMPTY_VALUE)
        {
         if(BuyPosOpen) if((Clr[0]<2 && Clr[0]!=EMPTY_VALUE) || Clr[0]==EMPTY_VALUE) BUY_Open=true;
         if(SellPosClose)SELL_Close=true;
         UpSignalTime=datetime(SeriesInfoInteger(Symbol(),InpInd_Timeframe,SERIES_LASTBAR_DATE))+TimeShiftSec;
        }
      //---- получим сигналы для продажи
      if(Clr[1]<2 && Clr[1]!=EMPTY_VALUE)
        {
         if(SellPosOpen) if((Clr[0]>1 && Clr[0]!=EMPTY_VALUE) || Clr[0]==EMPTY_VALUE) SELL_Open=true;
