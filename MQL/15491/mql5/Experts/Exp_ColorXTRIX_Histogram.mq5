...
int OnInit()
  {
//---- получение хендла индикатора ColorXTRIX_Histogram
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"ColorXTRIX_Histogram",XMA_Method,XLength,XPhase,Smooth,Mom_Period,IPC,0);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора ColorXTRIX_Histogram");
      return(INIT_FAILED);
     }
...
//---- Инициализация переменных начала отсчёта данных
   ...
   min_rates_total*=2;
   min_rates_total+=int(Mom_Period)+1;
   min_rates_total+=GetStartBars(XMA_Method,Smooth,XPhase);
