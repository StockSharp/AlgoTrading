...
#property description "trail its stop loss."
#property strict
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
...
input on_off_mode trail_sl=0; //Trail stop loss
...
//Trail stop loss
   Comment(OrderExist()," ",EnumToString(trail_sl)," ",OrderStopLoss());

   if(OrderExist()!=-1 && EnumToString(trail_sl)=="on" && OrderStopLoss()!=0)
     {
      if(OrderType()==OP_BUY)
        {
         if(OrderStopLoss()<Bid-sl*Point && OrderStopLoss()<Bid-sl*Point-slip*Point) if(OrderModify(OrderTicket(),OrderOpenPrice(),Bid-sl*Point,OrderTakeProfit(),0)==false) return;
        }
      else
        {
         if(OrderStopLoss()>Ask+sl*Point && OrderStopLoss()>Ask+sl*Point+slip*Point) if(OrderModify(OrderTicket(),OrderOpenPrice(),Ask+sl*Point,OrderTakeProfit(),0)==false) return;
        }
     }
