...
extern double MinProfit =  0.0025;
...
extern double MarginUsage = 0.08;
...
/*
void longStop(){
   double symbolHistory = 0;
   double totalHistoryProfit = 0;
    
   for( int iPos = OrdersHistoryTotal() - 1 ; iPos > ( OrdersHistoryTotal() - 1 ) - totalHistory; iPos-- ){
      OrderSelect( iPos, SELECT_BY_POS, MODE_HISTORY ) ;
      double QueryHistoryDouble = ( double ) QueryHistory;
      if( symbolHistory >= 50 ) break;
      if( OrderSymbol() == Symbol() ){
         totalHistoryProfit = totalHistoryProfit + OrderProfit() ;
         symbolHistory = symbolHistory + 1 ;
      }
   }
   if( totalHistoryProfit > 0 && symbolHistory >= 50 && ( totalProfit + totalLoss ) < 0 && MathAbs( totalProfit + totalLoss ) > 0.5 * totalHistoryProfit ){
      closeAll();
   }
}
*/
