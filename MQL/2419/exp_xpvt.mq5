...
int OnInit()
  {
//--- получение хендла индикатора XPVT
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"XPVT",VolumeType,XMA_Method,XLength,XPhase,IPC,0);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора XPVT");
      return(INIT_FAILED);
     }
//--- инициализация переменной для хранения периода графика в секундах  
   TimeShiftSec=PeriodSeconds(InpInd_Timeframe);
//--- инициализация переменных начала отсчёта данных
   min_rates_total=GetStartBars(XMA_Method,XLength,XPhase)+1+XLength;
