// The Stops Bot with RSI & Momentum
// Patrick Burns
...
int init()
  {
   ...
   if(MarketInfo(Symbol(),MODE_STOPLEVEL)>StopOrderDistance)
      Alert("StopOrderDistance points are not set correctly!");
...
int start()
  {
   ...
   if(Time[0]>lastActTime)
     {
      double d_Momentum=iMomentum(Symbol(),0,Momentum_Period,PRICE_CLOSE,1);
      ...
      if(orders_BUY+orders_SELL<MaxOrders)
        {
         if(isTradeTime)
           {
            if(BuyAllow && orders_BUYSTOP==0 && d_Momentum < Momentum_Buy_Restrict && d_RSI < RSI_Buy_Restrict) 
              {
               lastBuyStopTicket=DoPendingLot(Symbol(),LONG,OP_BUYSTOP,Open[0]+(Ask-Bid)+StopOrderDistance*Point,OrderSize,StopLoss,TakeProfit,"Stop Buy");
              }
            if(SellAllow && orders_SELLSTOP==0 && d_Momentum > Momentum_Sell_Restrict && d_RSI > RSI_Sell_Restrict)
              {
               lastSellStopTicket=DoPendingLot(Symbol(),SHORT,OP_SELLSTOP,Open[0]-StopOrderDistance*Point,OrderSize,StopLoss,TakeProfit,"Stop Sell");
              }
