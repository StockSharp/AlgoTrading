...
//+------------------------------------------------+
//| Входные параметры индикатора Go                |
//+------------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H1; //таймфрейм индикатора Go
input int Period_=174;                            // Период индикатора 
...
int OnInit()
  {
//---- получение хендла индикатора Go
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"Go",Period_,0);
   if(InpInd_Handle==INVALID_HANDLE) Print(" Не удалось получить хендл индикатора Go");
