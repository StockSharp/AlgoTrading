...
//---- getting handle of the 2pbIdeal1MA indicator
   InpInd1_Handle=iCustom(Symbol(),InpInd_Timeframe,"2pbIdeal1MA",Period1,Period2,0);
   if(InpInd1_Handle==INVALID_HANDLE) Print("Failed to get handle of the 2pbIdeal1MA indicator");
   
//---- getting handle of the 2pbIdeal3MA indicator
   InpInd2_Handle=iCustom(Symbol(),InpInd_Timeframe,"2pbIdeal3MA",PeriodX1,PeriodX2,PeriodY1,PeriodY2,PeriodZ1,PeriodZ2,0);
   if(InpInd2_Handle==INVALID_HANDLE) Print("Failed to get handle of the 2pbIdeal3MA indicator");
   ...
   if(!SignalBar || NB.IsNewBar(Symbol(),InpInd_Timeframe) || Recount) // checking for a new bar
     {
      ...
      if(CopyBuffer(InpInd1_Handle,0,SignalBar,2,Ma1)<=0) {Recount=true; return;}
      if(CopyBuffer(InpInd2_Handle,0,SignalBar,2,Ma2)<=0) {Recount=true; return;}

      //---- Getting buy signals
      if(Ma1[1]>Ma2[1])
        {
         if(BuyPosOpen && Ma1[0]<Ma2[0]) BUY_Open=true;
      ...
      if(Ma1[1]<Ma2[1])
        {
         if(SellPosOpen && Ma1[0]>Ma2[0]) SELL_Open=true;
