//+------------------------------------------------------------------+
//|                                 Exp_AroonOscillatorSignAlert.mq5 |
...
//+----------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H4; //таймфрейм индикатора AroonOscillatorSignAlert
uint ATR_Period=14;
...
int OnInit()
  {
//---- получение хендла индикатора AroonOscillatorSignAlert
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"AroonOscillatorSignAlert",ATR_Period,AroonPeriod,UpLevel,DnLevel,SignalBar,SoundON,NumberofAlerts,EMailON,PushON);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора AroonOscillatorSignAlert");
      return(INIT_FAILED);
     }
