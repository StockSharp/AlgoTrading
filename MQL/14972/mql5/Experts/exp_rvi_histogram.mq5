...
//+----------------------------------------------+
//| Входные параметры индикатора RVI             |
//+----------------------------------------------+
...
input uint RVIPeriod=14;                          // Период индикатора
input double HighLevel=+0.3;                      // Уровень перекупленности
...
int OnInit()
  {
//---- получение хендла индикатора RVI_Histogram
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"RVI_Histogram",RVIPeriod,HighLevel,LowLevel,0);
   if(InpInd_Handle==INVALID_HANDLE) Print(" Не удалось получить хендл индикатора RVI_Histogram");
