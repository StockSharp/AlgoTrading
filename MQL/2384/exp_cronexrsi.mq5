...
input uint RSIPeriod=25;                         // Период индикатора RSI
input ENUM_APPLIED_PRICE   RSIPrice=PRICE_CLOSE; // Цена индикатора RSI
...
int OnInit()
  {
//--- получение хендла индикатора CronexRSI
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"CronexRSI",RSIPeriod,RSIPrice,XMA_Method,FastPeriod,SlowPeriod,XPhase);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора CronexRSI");
      return(INIT_FAILED);
     }
