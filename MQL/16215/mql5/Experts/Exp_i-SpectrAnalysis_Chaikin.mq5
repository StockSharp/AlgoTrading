//+------------------------------------------------------------------+
//|                                 Exp_i-SpectrAnalysis_Chaikin.mq5 |
...
int OnInit()
  {
//---- получение хендла индикатора i-SpectrAnalysis_Chaikin
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"i-SpectrAnalysis_Chaikin",fast_ma_period,slow_ma_period,ma_method,VolumeType,N,SS,0);
   if(InpInd_Handle==INVALID_HANDLE)  
     {
      Print(" Не удалось получить хендл индикатора i-SpectrAnalysis_Chaikin");
      return(INIT_FAILED);
     }
...
//---- Инициализация переменных начала отсчёта данных
   int tnn1=int(MathPow(2,N));
   int M=tnn1+1;
   min_rates_total=int(M+MathMax(fast_ma_period,slow_ma_period));
