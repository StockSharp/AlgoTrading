...
//+----------------------------------------------------+
//| Входные параметры индикатора Laguerre              |
//+----------------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H1; //таймфрейм индикатора Laguerre
...
int OnInit()
  {
//---- получение хендла индикатора ColorLaguerre
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"ColorLaguerre",gamma,HighLevel,MiddleLevel,LowLevel);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора ColorLaguerre");
      return(INIT_FAILED);
     }
