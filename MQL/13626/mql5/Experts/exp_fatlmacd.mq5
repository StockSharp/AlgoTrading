...
//+----------------------------------------------+
//| Входные параметры индикатора FatlMacd        |
//+----------------------------------------------+
...
input uint XMA_Lengh = 20;                           // Период усреднения
...
int OnInit()
  {
//---- получение хендла индикатора FatlMacd
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"FatlMacd",XMA_Method,XMA_Lengh,XMA_Phase,AppliedPrice);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора FatlMacd");
      return(INIT_FAILED);
     }
...
//---- инициализация переменных начала отсчета данных
   min_rates_total+=39;
   min_rates_total+=GetStartBars(XMA_Method,XMA_Lengh,XMA_Phase);
