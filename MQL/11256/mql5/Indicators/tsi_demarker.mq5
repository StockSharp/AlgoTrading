...
#property description "TSI_DeMarker"
...
#property indicator_label1  "TSI_DeMarker"
//+----------------------------------------------+
...
input uint XLength3=12;                           // глубина усреднения сигнальной линии
...
//--- создание имени для отображения в отдельном подокне и во всплывающей подсказке
   IndicatorSetString(INDICATOR_SHORTNAME,"TSI_DeMarker");
...
//---- объявления локальных переменных 
   double DeMarker[],mtm,xmtm,xxmtm,absmtm,xabsmtm,xxabsmtm,tsi,xtsi;
...
//---- копируем вновь появившиеся данные в массивы
   if(CopyBuffer(Ind_Handle,0,0,to_copy,DeMarker)<=0) return(RESET);
//----  
...
//---- основной цикл расчёта индикатора
   for(bar=limit; bar>=0 && !IsStopped(); bar--)
     {
      mtm=Mom.MomentumSeries(maxbar1,prev_calculated,rates_total,MomPeriod,DeMarker[bar],bar,true);
      absmtm=MathAbs(mtm);
