//+------------------------------------------------------------------+
//|                               Exp_ColorSchaffJJRSXTrendCycle.mq5 |
...
//+------------------------------------------------------------+
//| Входные параметры индикатора ColorSchaffJJRSXTrendCycle    |
//+------------------------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H4; // Таймфрейм индикатора ColorSchaffJJRSXTrendCycle
...
int OnInit()
  {
//---- получение хендла индикатора ColorSchaffJJRSXTrendCycle
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"ColorSchaffJJRSXTrendCycle",Fast_JJRSX,Slow_JJRSX,Smooth,JPhase,IPC,0,Cycle,HighLevel,MiddleLevel,LowLevel);
   if(InpInd_Handle==INVALID_HANDLE)
    {
     Print(" Не удалось получить хендл индикатора ColorSchaffJJRSXTrendCycle");
     return(INIT_FAILED);
     }
