...
//+----------------------------------------------+
//| Beginner indicator input parameters          |
//+----------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H4; //time frame of the Beginner indicator
//----
...
int OnInit()
  {
//---- getting the Beginner indicator handle
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"Beginner",Otstup,Per);
   if(InpInd_Handle==INVALID_HANDLE) Print(" Failed to get handle of Beginner indicator!");
   ...
   if(InpInd_Timeframe==Period())
     {
      ...
      //---- add the Beginner indicator on the chart  
      if(!cchart.IndicatorAdd(0,InpInd_Handle)) Print(" Failed to add the Beginner indicator on the chart!");
     }
...
//---- delete the indicator from the chart
   ...
   if(InpInd_Timeframe==Period())
      if(!IndicatorRelease(InpInd_Handle)
         || !ChartIndicatorDelete(0,0,short_name))
         Print(" Failed to delete the Beginner indicator from the chart!");
//----
  }
//+------------------------------------------------------------------+
