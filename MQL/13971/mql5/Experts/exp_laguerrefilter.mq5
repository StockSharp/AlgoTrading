...
//+--------------------------------------------------+
//| Входные параметры для индикатора LaguerreFilter  |
//+--------------------------------------------------+
...
int OnInit()
  {
//---- получение хендла индикатора LaguerreFilter
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"LaguerreFilter",Gamma,0);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print("Не удалось получить хендл индикатора LaguerreFilter");
      return(INIT_FAILED);
     }
   ...
   if(!SignalBar || NB.IsNewBar(Symbol(),InpInd_Timeframe) || Recount) // проверка на появление нового бара
     {
      ...
      if(CopyBuffer(InpInd_Handle,1,SignalBar,2,Osc1)<=0) {Recount=true; return;}
      if(CopyBuffer(InpInd_Handle,0,SignalBar,2,Osc2)<=0) {Recount=true; return;}
