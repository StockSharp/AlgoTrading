   ...
   {   
      ...
      {        
         if (isTradeTime)
         {
            ...
            {
               lastBuyStopTicket = DoPendingLot(Symbol(), LONG, OP_BUYLIMIT, Open[0] - (Ask - Bid) - StopOrderDistance*Point, OrderSize, StopLoss, TakeProfit, "Bertie The Bat Buy");
            }
            if (SellAllow && orders_SELLSTOP == 0)
            {
               lastSellStopTicket = DoPendingLot(Symbol(), SHORT, OP_SELLLIMIT, Open[0] + StopOrderDistance*Point, OrderSize, StopLoss, TakeProfit, "Bertie The Bat Sell");
            }
