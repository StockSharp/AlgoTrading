...
//+----------------------------------------------+
//| Входные параметры индикатора iDeMarkerSign   |
//+----------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H4; //таймфрейм индикатора iDeMarkerSign
uint ATR_Period=14;
...
int OnInit()
  {
//---- получение хендла индикатора iDeMarkerSign
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"iDeMarkerSign",ATR_Period,iDeMarkerPeriod,UpLevel,DnLevel);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора iDeMarkerSign");
      return(INIT_FAILED);
     }
