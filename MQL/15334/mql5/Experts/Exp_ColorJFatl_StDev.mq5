...
int OnInit()
  {
//---- получение хендла индикатора ColorJFatl_StDev
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"ColorJFatl_StDev",JLength,JPhase,IPC,PriceShift,dK1,dK2,std_period,0);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора ColorJFatl_StDev");
      return(INIT_FAILED);
     }
...
//---- Инициализация переменных начала отсчёта данных
   int FATLSize=39;
   min_rates_total=FATLSize+30+int(std_period);
