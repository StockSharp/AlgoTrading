	    ...
	    if(DcF>0)
     	 {
      	 ...
      	 if(losses>1) 
      	    lot=NormalizeDouble(lot-lot*losses/DcF,1);
     	 }
    }
//---- return lot size
   if(lot<0.1) lot=0.1;
   ...
   if (TimeHour(Time[1])== 0 )
     {
      ...
      hLine = High[tHour];
         ...
         if (isTrace == true)
           {
            Print("i# ",i," lLine# ", lLine, " Low# ", Low[i+1], " hLine #", hLine, " High #", 
                  High[i+1], " time #" , TimeHour(Time[i+1]), " day #", TimeDay(Time[i+1]));
           }
        }
      ...
      if (err < 0)
        {
         ...
         if (GetLastError()== 130) 
            err2 =OrderSend(Symbol(),OP_SELL,LotsOptimized(),Bid,3,Resist2,Support3,"",MAGICMA,0,Red);
         if ( err2 <0)
           {
      ...
      if (err < 0)
        {
         ...
         if (GetLastError() == 130) 
            err2=OrderSend(Symbol(),OP_BUY,LotsOptimized(),Ask,3,Support2,Resist3,"",MAGICMA,0,Blue);
         StpBuy  = Support2;
         PrftBuy = Resist2;
         if (err2 < 0)
           {
   ...
   if (isTrace== true)
     {
      if (RetVal!= ERR_NO_MQLERROR) 
         Print ("Error, message not sending: ", ErrorDescription(RetVal));
     }
  }
//+------------------------------------------------------------------+
