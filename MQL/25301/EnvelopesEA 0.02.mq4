#property strict                                //A lot size of 3.6 was used during testing on EUR/USD 4HRs.
...
void OnTick()
{
   ...
   if (OrdersTotal() == 0 && Signal == "S"){int s0 = OrderSend(_Symbol,OP_SELL,Lots,Bid,0,0,0,NULL,0,clrIndigo);SellIsActive = True; Price = Bid;}
   if (OrdersTotal() == 0 && Signal == "B"){int b0 = OrderSend(_Symbol,OP_BUY ,Lots,Ask,0,0,0,NULL,0,clrIndigo);BuyIsActive = True;Price = Bid;} 
   if (OrdersTotal() == 1 && SellIsActive == True && Bid - Price > 100*_Point){int s0 = OrderSend(_Symbol,OP_SELL,Lots,Bid,0,0,Bid-200*_Point,NULL,0,clrIndigo);SellIsActive = True; Price = Bid;}
   if (OrdersTotal() == 1 && BuyIsActive == True && Price - Bid > 100*_Point){int b0 = OrderSend(_Symbol,OP_BUY ,Lots,Ask,0,0,Ask+200*_Point,NULL,0,clrIndigo);BuyIsActive = True;Price = Ask;}   
   if (OrdersTotal() == 2 && SellIsActive == True && Bid - Price > 200*_Point){int s0 = OrderSend(_Symbol,OP_SELL,Lots,Bid,0,0,Bid-400*_Point,NULL,0,clrIndigo);SellIsActive = True; Price = Bid;}
   if (OrdersTotal() == 2 && BuyIsActive == True && Price - Bid > 200*_Point){int b0 = OrderSend(_Symbol,OP_BUY ,Lots,Ask,0,0,Ask+400*_Point,NULL,0,clrIndigo);BuyIsActive = True;Price = Ask;}     
   if (OrdersTotal() == 3 && SellIsActive == True && Bid - Price > 300*_Point){int s0 = OrderSend(_Symbol,OP_SELL,Lots,Bid,0,0,Bid-400*_Point,NULL,0,clrIndigo);SellIsActive = True; Price = Bid;}
   if (OrdersTotal() == 3 && BuyIsActive == True && Price - Bid > 300*_Point){int b0 = OrderSend(_Symbol,OP_BUY ,Lots,Ask,0,0,Ask+400*_Point,NULL,0,clrIndigo);BuyIsActive = True;Price = Ask;}     
   
   if(Signal == "E"){CloseTrades();BuyIsActive = False;SellIsActive = False;}
...
{  
   string Signal = "";
   double U = iEnvelopes(_Symbol,_Period,50,MODE_EMA,0,PRICE_CLOSE,0.58,MODE_UPPER,0);
   double L = iEnvelopes(_Symbol,_Period,50,MODE_EMA,0,PRICE_CLOSE,0.58,MODE_LOWER,0);    
   if(Bid > U){Signal = "S";}
   if(Bid < L){Signal = "B";}
   if(BuyIsActive == True && Bid > U){Signal = "E";}
   if(SellIsActive == True && Bid < L){Signal = "E";}
   return(Signal);   
}
   ...
   //if(BuyIsActive == True && AccountEquity() > AccountBalance() + (AccountBalance() * (Lots/10))){CloseTrades();BuyIsActive = False;} 
   //if(SellIsActive == True && AccountEquity() > AccountBalance() + (AccountBalance() * (Lots/10))){CloseTrades();SellIsActive = False;}
