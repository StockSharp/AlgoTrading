#property strict                                // Lot size of 3.6 was used during testing.
...
{  
   string Signal = "";
   double U = iEnvelopes(_Symbol,_Period,50,MODE_EMA,0,PRICE_CLOSE,0.5,MODE_UPPER,0);
   double L = iEnvelopes(_Symbol,_Period,50,MODE_EMA,0,PRICE_CLOSE,0.5,MODE_LOWER,0);    
   if (Bid - 100*_Point > U){Signal = "S";}
   if (Bid + 100*_Point < L){Signal = "B";}
   return(Signal);   
}
