...
void OnTick()
{
   ...
   if(BuyIsActive == True && AccountEquity() > AccountBalance() + (AccountBalance() * Lots)){CloseTrades();BuyIsActive = False;} 
   if(SellIsActive == True && AccountEquity() > AccountBalance() + (AccountBalance() * Lots)){CloseTrades();SellIsActive = False;}
