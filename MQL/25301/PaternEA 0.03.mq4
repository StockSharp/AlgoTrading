...
void OnTick()
{
   ...
   if (Signal == "S" && SellIsActive == False){int s0 = OrderSend(_Symbol,OP_SELL,Lots*20,Bid,0,0,Bid-2500*_Point,NULL,0,clrIndigo);SellIsActive = True; Price = Bid;}
   if (Signal == "B" && BuyIsActive == False){int b0 = OrderSend(_Symbol,OP_BUY ,Lots*20,Ask,0,0,Ask+800*_Point,NULL,0,clrIndigo);BuyIsActive = True;Price = Bid;}
 
   if(rsi < 20 && Ask < ma1 - 400*_Point){Signal = "B";}    
   if(rsi > 80 && Bid > ma1 + 400*_Point){Signal = "S";} 
   
   if(BuyIsActive == True && AccountEquity() > AccountBalance() * 2){CloseTrades();BuyIsActive = False;} 
   if(SellIsActive == True && AccountEquity() > AccountBalance() * 2){CloseTrades();SellIsActive = False;}
