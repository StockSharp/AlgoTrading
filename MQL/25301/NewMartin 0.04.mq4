...
{  
   ...
   Comment(" Orders: ", OrdersTotal(),"    ", " Bid: ", Bid, "    ", " Equity: ", AccountEquity(), "    ", " S :" , _Symbol);
   ...
   if(OrdersTotal() == 1 && AccountEquity() < AccountBalance() * 0.01){int b2 = OrderSend(_Symbol,OP_BUY,Lots,Ask,0,0,0,NULL,0,0,0);BuyIsActive = True;}
                 
   if(AccountEquity() > AccountBalance() * 1.3){CloseTrades();BuyIsActive = False;SellIsActive = False;}
   if(_Symbol == "NZDUSD" && AccountEquity() > AccountBalance() * 1.02){CloseTrades();BuyIsActive = False;SellIsActive = False;}   
}  
