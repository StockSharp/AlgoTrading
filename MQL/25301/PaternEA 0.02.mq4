...
void OnTick()
{
   ...
   double avg = iMA(_Symbol,_Period,200,0,MODE_SMA,PRICE_LOW,0);

   Comment(" Orders: ", OrdersTotal() ,"    ", " Balance: ", AccountBalance(), "    ", " Equity: ", AccountEquity(), "    " , " Signal: " ,Signal);
   
   if (OrdersTotal() == 0 && Signal == "S"){int s0 = OrderSend(_Symbol,OP_SELL,Lots,Bid,0,0,0,0,0,clrIndigo);SellIsActive = True;}
   if (OrdersTotal() == 0 && Signal == "B"){int b0 = OrderSend(_Symbol,OP_BUY ,Lots,Ask,0,0,0,0,0,clrIndigo);BuyIsActive = True;}
   
   if (SellIsActive == True && AccountEquity() < AccountBalance()-(AccountBalance()*0.1)){int s1 = OrderSend(_Symbol,OP_SELL,Lots,Bid,0,0,0,0,0,clrIndigo);SellIsActive = True;}
   if (BuyIsActive  == True && AccountEquity() < AccountBalance()-(AccountBalance()*0.1)){int b1 = OrderSend(_Symbol,OP_BUY ,Lots,Ask,0,0,0,0,0,clrIndigo);BuyIsActive = True;}    

   if(BuyIsActive  == True && AccountEquity() > AccountBalance()+(AccountBalance()*0.2)){CloseTrades();BuyIsActive = False;} 
   if(SellIsActive == True && AccountEquity() > AccountBalance()+(AccountBalance()*0.2)){CloseTrades();SellIsActive = False;}
