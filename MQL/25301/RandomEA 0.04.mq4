#property strict                                //Thise settings work on EURUSD with a 50,000 dollars account
...
void OnTick()
{
   ...
   if (OrdersTotal() == 1 && SellIsActive == True && Bid > Price + 300*_Point){int s1 = OrderSend(_Symbol,OP_SELL,Lots*2,Bid,0,0,0,NULL,0,clrIndigo);SellIsActive = True; Price = Bid;}
   if (OrdersTotal() == 1 && BuyIsActive == True && Price-Ask > 300*_Point){int b1 = OrderSend(_Symbol,OP_BUY ,Lots*2,Ask,0,0,0,NULL,0,clrIndigo);BuyIsActive = True;Price = Bid;} 
   if (OrdersTotal() == 2 && SellIsActive == True && Bid > Price + 300*_Point){int s1 = OrderSend(_Symbol,OP_SELL,Lots*3,Bid,0,0,0,NULL,0,clrIndigo);SellIsActive = True; Price = Bid;}
   if (OrdersTotal() == 2 && BuyIsActive == True && Price-Ask > 300*_Point){int b1 = OrderSend(_Symbol,OP_BUY ,Lots*3,Ask,0,0,0,NULL,0,clrIndigo);BuyIsActive = True;Price = Bid;} 
   if (OrdersTotal() == 3 && SellIsActive == True && Bid > Price + 300*_Point){int s1 = OrderSend(_Symbol,OP_SELL,Lots*4,Bid,0,0,0,NULL,0,clrIndigo);SellIsActive = True; Price = Bid;}
   if (OrdersTotal() == 3 && BuyIsActive == True && Price-Ask > 300*_Point){int b1 = OrderSend(_Symbol,OP_BUY ,Lots*4,Ask,0,0,0,NULL,0,clrIndigo);BuyIsActive = True;Price = Bid;}   
   if (OrdersTotal() == 4 && SellIsActive == True && Bid > Price + 300*_Point){int s1 = OrderSend(_Symbol,OP_SELL,Lots*5,Bid,0,0,0,NULL,0,clrIndigo);SellIsActive = True; Price = Bid;}
   if (OrdersTotal() == 4 && BuyIsActive == True && Price-Ask > 300*_Point){int b1 = OrderSend(_Symbol,OP_BUY ,Lots*5,Ask,0,0,0,NULL,0,clrIndigo);BuyIsActive = True;Price = Bid;}  
...
{  
   string Signal = "";
   if (Close[1] > Open[1]){Signal = "S";}
   if (Close[1] < Open[1]){Signal = "B";}
   return(Signal);   
}
