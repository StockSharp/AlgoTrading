...
void OnTick()
{
   ...
   if(OrdersTotal() == 0 && Signal == "S"){int s0 = OrderSend(_Symbol,OP_SELL,Lots,Bid,0,0,0,NULL,0,clrRed);SellIsActive = True; Price = Bid;}
   if(OrdersTotal() == 0 && Signal == "B"){int b0 = OrderSend(_Symbol,OP_BUY ,Lots,Ask,0,0,0,NULL,0,clrGreen);BuyIsActive = True;Price = Bid;} 
   if(Signal == "E"){CloseTrades();SellIsActive = False;BuyIsActive = False;} 
}  
