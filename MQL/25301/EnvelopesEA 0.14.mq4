#property strict                                // Tested on EUR/USD 2019
static bool BuyIsActive = False;
static bool SellIsActive = False;
extern double UD = 0.5;
extern double LD = 0.5;
extern double Lots = 0.2;

void OnTick()
{
   //double Lots = NormalizeDouble(AccountBalance()/2500,2);

   string Signal = GetSignal();
        
   Comment(" Orders: ", OrdersTotal() ,"    ", " Balance: ", AccountBalance(), "    ", " Equity: ", AccountEquity(), "    " , " Signal: " ,Signal,"    ");
   
   if (OrdersTotal() == 0 && Signal == "S"){int s1 = OrderSend(_Symbol,OP_SELL,Lots,Bid,0,0,0,NULL,0,clrIndigo);SellIsActive = True;}
   if (OrdersTotal() == 0 && Signal == "B"){int b1 = OrderSend(_Symbol,OP_BUY ,Lots,Ask,0,0,0,NULL,0,clrIndigo);BuyIsActive = True;}
...
{  
   string Signal = "N";
   double U = iEnvelopes(_Symbol,_Period,50,MODE_SMA,0,PRICE_CLOSE,UD,MODE_UPPER,0);
   double L = iEnvelopes(_Symbol,_Period,50,MODE_SMA,0,PRICE_CLOSE,LD,MODE_LOWER,0);
