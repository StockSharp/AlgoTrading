...
//+----------------------------------------------+
//| Входные параметры индикатора BykovTrend      |
//+----------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H1; //таймфрейм индикатора BykovTrend
input int  RISK=3;                                //степень риска
input int  SSP=9;                                 //период WPR
...
int OnInit()
  {
//---- получение хендла индикатора BykovTrend
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"BykovTrend",RISK,SSP);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора BykovTrend");
      return(INIT_FAILED);
     }
