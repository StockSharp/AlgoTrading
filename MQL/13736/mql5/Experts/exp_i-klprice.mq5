...
input Smooth_Method MA_Method2=MODE_JJMA; // Метод усреднения размера свечей
input uint Length2=20; // Глубина усреднения размера свечей
input int Phase2=100;  // Параметр сглаживания размера свечей
                       // для JJMA изменяющийся в пределах -100 ... +100, влияет на качество переходного процесса
// для VIDIA это период CMO, для AMA это период медленной скользящей
...
int OnInit()
  {
//---- получение хендла индикатора i-KlPrice
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"i-KlPrice",MA_Method1,Length1,Phase1,MA_Method2,Length2,Phase2,Deviation,Smooth,IPC,UpLevel,DnLevel,0);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора i-KlPrice");
      return(INIT_FAILED);
     }
   ...
   if(!SignalBar || NB.IsNewBar(Symbol(),InpInd_Timeframe) || Recount) // проверка на появление нового бара
     {
      ...
      if(SellPosClose && Value[1]>2) SELL_Close=true;
