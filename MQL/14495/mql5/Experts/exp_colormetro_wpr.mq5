...
//+----------------------------------------------+
//| Входные параметры индикатора ColorMETRO_WPR  |
//+----------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H8; // Таймфрейм индикатора
//----
input int PeriodWPR=7;// Период индикатора
...
int OnInit()
  {
//---- получение хендла индикатора ColorMETRO_WPR
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"ColorMETRO_WPR",PeriodWPR,StepSizeFast,StepSizeSlow);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора ColorMETRO_WPR");
      return(INIT_FAILED);
     }
