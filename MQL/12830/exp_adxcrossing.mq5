...
//+----------------------------------------------+
//| Входные параметры индикатора ADXCrossing     |
//+----------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H1; // Таймфрейм индикатора ADXCrossing
...
int OnInit()
  {
//--- получение хендла индикатора ADXCrossing
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"ADXCrossing",ADXPeriod);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора ADXCrossing");
      return(INIT_FAILED);
     }
