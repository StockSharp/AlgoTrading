//+------------------------------------------------------------------+
//|                                 Exp_ColorSchaffMFITrendCycle.mq5 |
...
//+-------------------------------------------------------+
//| Входные параметры индикатора ColorSchaffMFITrendCycle |
//+-------------------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H1; // Таймфрейм индикатора ColorSchaffMFITrendCycle
...
int OnInit()
  {
//---- получение хендла индикатора ColorSchaffMFITrendCycle
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"ColorSchaffMFITrendCycle",Fast_MFI,Slow_MFI,VolumeType,Cycle,HighLevel,0,LowLevel);
   if(InpInd_Handle==INVALID_HANDLE)
    {
     Print(" Не удалось получить хендл индикатора ColorSchaffMFITrendCycle");
     return(INIT_FAILED);
     }
//---- инициализация переменной для хранения периода графика в секундах  
   TimeShiftSec=PeriodSeconds(InpInd_Timeframe);
//---- инициализация переменных начала отсчета данных
   int min_rates_1=int(Fast_MFI);
   int min_rates_2=int(Slow_MFI);
