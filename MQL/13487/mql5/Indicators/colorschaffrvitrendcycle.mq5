//+------------------------------------------------------------------+
//|                                     ColorSchaffRVITrendCycle.mq5 |
//|                                  Copyright © 2011, EarnForex.com |
//|                                        http://www.earnforex.com/ |
//+------------------------------------------------------------------+ 
#property copyright "Copyright © 2011, EarnForex.com"
#property link      "http://www.earnforex.com"
#property description "Schaff Trend Cycle - Cyclical Stoch over Stoch over RVI MACD."
...
#property indicator_color1 clrRed,clrViolet,clrGold,clrDarkOrchid,clrMediumBlue,clrAqua,clrLightSkyBlue,clrDodgerBlue
//---- линия индикатора - сплошная
#property indicator_style1 STYLE_SOLID
//---- толщина линии индикатора равна 2
#property indicator_width1 2
//---- отображение метки индикатора
#property indicator_label1 "Schaff RVI Trend Cycle"
//+-----------------------------------------------+
...
//+-----------------------------------------------+
input uint Fast_RVI = 23; // Период быстрого RVI
input uint Slow_RVI = 50; // Период медленного RVI
...
//---- инициализация переменных начала отсчета данных
   min_rates_1=int(Fast_RVI);
   min_rates_2=int(Slow_RVI);
   min_rates_total=3*MathMax(min_rates_1,min_rates_2)+int(Cycle)+1;
//--- получение хендла индикатора MA 1
   Ind1_Handle=iRVI(Symbol(),PERIOD_CURRENT,Fast_RVI);
   if(Ind1_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора RVI 1");
      return(INIT_FAILED);
     }
//--- получение хендла индикатора MA 2
   Ind2_Handle=iRVI(Symbol(),PERIOD_CURRENT,Slow_RVI);
   if(Ind2_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора RVI 2");
      return(INIT_FAILED);
     }
...
//---- инициализация переменной для короткого имени индикатора
   string shortname;
   StringConcatenate(shortname,"Schaff RVI Trend Cycle( ",Fast_RVI,", ",Slow_RVI,", ",Cycle," )");
...
//---- объявление переменных с плавающей точкой  
   double fastRVI[],slowRVI[],LLV,HHV;
...
//---- копируем вновь появившиеся данные в массивы
   if(CopyBuffer(Ind1_Handle,0,0,to_copy,fastRVI)<=0) return(RESET);
   if(CopyBuffer(Ind2_Handle,0,0,to_copy,slowRVI)<=0) return(RESET);
   ...
   for(bar=limit; bar>=0 && !IsStopped(); bar--)
     {
      ...
      Bar1=Count[1];
      //----
      MACD[Bar0]=fastRVI[bar]-slowRVI[bar];
      //----
