...
//+----------------------------------------------+
//| Входные параметры индикатора JMACandleSign   |
//+----------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H4; // Таймфрейм индикатора JMACandleSign
...
int OnInit()
  {
//---- получение хендла индикатора JMACandleSign
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"JMACandleSign",JLength,JPhase);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора JMACandleSign");
      return(INIT_FAILED);
     }
