...
//+------------------------------------------------------------------+
#property copyright "Copyright 2016, Roberto Jacobs (3rjfx) ~ By 3rjfx ~ Created: 2016/12/05"
#property link      "http://www.mql5.com"
#property link      "https://www.mql5.com/en/users/3rjfx"
#property version   "1.00"
//--
#property description "The ExpHAWaves is the MT5 Expert Advisor based on the Heiken Ashi and StepUpDown Indicator."
//---
//+------------------------------------------------------------------+
//|                             Include                              |
//+------------------------------------------------------------------+
...
//---
    ...
    int i,j,pr=9;
    //--
    if(!FillArrayMA(sma01ar,0,sma01h,ebar)) return(0);
    if(!FillArrayMA(mma30ar,0,mma30h,ebar)) return(0);
    //--
    bool ups=false;
    bool dns=false;
    //--
...
//---
    ArrayResize(ExOpen,bar);
    ArrayResize(ExClose,bar);
    ArraySetAsSeries(ExOpen,false);
    ArraySetAsSeries(ExClose,false);
    //--
    vbar=Bars(_Symbol,TimeFrames);;
    int hab=vbar!=wbar ? ebar : bar;
    //--
//---
    FillArraysOHLC(period,hab);
//---
    ExOpen[0]=OPEN[0];
    ExClose[0]=CLOSE[0];
    //--
...
//--- main loop of calculations
   for(i=pos; i<bar; i++)
     {
       haOpen=(ExOpen[i-1]+ExClose[i-1])/2;
       haClose=(OPEN[i]+HIGH[i]+LOW[i]+CLOSE[i])/4;
       //--
       if(haOpen<haClose) {hkn=1;}
       else if(haOpen>haClose){hkn=-1;}
       //--
       ExOpen[i]=haOpen;
       ExClose[i]=haClose;
     }
