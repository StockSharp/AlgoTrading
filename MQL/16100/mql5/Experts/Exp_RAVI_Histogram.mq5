...
//+----------------------------------------------+
//| Входные параметры индикатора PFE             |
//+----------------------------------------------+
...
input double UpLevel=+0.1;               //уровень перекупленности
input double DnLevel=-0.1;               //уровень перепроданности
//----
...
int OnInit()
  {
//---- получение хендла индикатора RAVI_Histogram
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"RAVI_Histogram",FastMethod,FastLength,FastPhase,SlowMethod,
                      SlowLength,SlowPhase,IPC,0,UpLevel,UpLevel,DnLevel,DnLevel);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора RAVI_Histogram");
      return(INIT_FAILED);
     }
