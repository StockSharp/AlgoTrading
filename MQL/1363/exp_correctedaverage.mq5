...
#property description "The breakthrough system based on the CorrectedAverage moving average"
//+-----------------------------------------------+
...
//+-----------------------------------------------+
//| CorrectedAverage indicator input parameters   |
//+-----------------------------------------------+
...
input int Level=300;//breakthrough level in points          
...
int OnInit()
  {
//---- getting the CorrectedAverage indicator handle
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"CorrectedAverage",MA_Method,Length,Applied_price,0,0);
   if(InpInd_Handle==INVALID_HANDLE) Print(" Failed to get handle of CorrectedAverage indicator");
...
//---- Initialization of breakthrough level in points of price chart
   dLevel=_Point*Level;
//----
   return(0);
  }
   ...
   if(!SignalBar || NB.IsNewBar(Symbol(),InpInd_Timeframe) || Recount) // checking for a new bar
     {
      ...
      //---- Getting buy signals
      if(Close[1]>Value[1]+dLevel)
        {
         if(BuyPosOpen&&Close[0]<=Value[0]+dLevel) BUY_Open=true;
      ...
      if(Close[1]<Value[1]-dLevel)
        {
         if(SellPosOpen&&Close[0]>=Value[0]-dLevel) SELL_Open=true;
