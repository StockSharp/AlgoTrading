...
//----
input int PeriodDeMarker=7; // Период индикатора
...
int OnInit()
  {
//---- получение хендла индикатора ColorMETRO_DeMarker
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"ColorMETRO_DeMarker",PeriodDeMarker,StepSizeFast,StepSizeSlow);
   if(InpInd_Handle==INVALID_HANDLE) Print(" Не удалось получить хендл индикатора ColorMETRO_DeMarker");
