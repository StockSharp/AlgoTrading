...
//+----------------------------------------------+
//| CMO indicator input parameters               |
//+----------------------------------------------+
...
//---- getting handle of the CMO indicator
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"CMO",Length,Method,Price,0);
   if(InpInd_Handle==INVALID_HANDLE) Print(" Failed to get handle of CMO indicator");

//---- initialization of a variable for storing a chart period in seconds  
   TimeShiftSec=PeriodSeconds(InpInd_Timeframe);

//---- Initialization of variables of the start of data calculation
   min_rates_total=int(Length+1+1+SignalBar);
//----
   return(0);
  }
//+------------------------------------------------------------------+
   ...
   if(!SignalBar || NB.IsNewBar(Symbol(),InpInd_Timeframe) || Recount) // checking for a new bar
     {
      ...
      if(Value[1]>0.0)
        {
         if(BuyPosOpen && Value[0]<0.0) BUY_Open=true;
      ...
      if(Value[1]<0.0)
        {
         if(SellPosOpen && Value[0]>0.0) SELL_Open=true;
