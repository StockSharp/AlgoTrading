...
//+----------------------------------------------+
//| Входные параметры индикатора Force_DiverSign |
//+----------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H4;  // Таймфрейм индикатора Force_DiverSign
...
int OnInit()
  {
//---- получение хендла индикатора Force_DiverSign
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"Force_DiverSign",MAType1,iPeriod1,MAType2,iPeriod2,VolumeType,0);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора Force_DiverSign");
      return(INIT_FAILED);
     }
...
//---- инициализация переменных начала отсчета данных
   int ATR_Period=10;
   min_rates_total=int(MathMax(iPeriod1,iPeriod2));
   min_rates_total+=int(MathMax(iPeriod1,iPeriod2))+5;
