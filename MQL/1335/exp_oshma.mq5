...
enum AlgMode
  {
   breakdown,  //breakthrough of zero by histogram
   twist       //changing the histogram direction
  };
//+----------------------------------------------+
...
int OnInit()
  {
//---- getting the OsHMA indicator handle
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"OsHMA",FastHMA,SlowHMA);
   if(InpInd_Handle==INVALID_HANDLE) Print(" Failed to get handle of OsHMA indicator");

//---- initialization of a variable for storing a chart period in seconds  
   TimeShiftSec=PeriodSeconds(InpInd_Timeframe);

//---- Initialization of variables of the start of data calculation
   int fHma2_Period=int(MathFloor(FastHMA/2));
   int sHma2_Period=int(MathFloor(SlowHMA/2));
   int fSqrt_Period=int(MathFloor(MathSqrt(FastHMA)));
   int sSqrt_Period=int(MathFloor(MathSqrt(SlowHMA)));
