...
//+------------------------------------------------+
//| Входные параметры индикатора NonLagDot         |
//+------------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H1; //таймфрейм индикатора NonLagDot
input ENUM_APPLIED_PRICE         Price=PRICE_CLOSE;//ценовая константа
input ENUM_MA_METHOD              Type=MODE_SMA; //метод усреднения
input uint                      Length=10; //период расчета индикатора
input uint                      Filter=0; //фильтр
input double                     Swing=0; //отклонение
...
//---- Инициализация констант
   double Fi=Filter*_Point;
//---- получение хендла индикатора NonLagDot
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"NonLagDot",Price,Type,Length,Filter,Swing,0);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора NonLagDot");
      return(INIT_FAILED);
     }
//---- инициализация переменной для хранения периода графика в секундах  
   TimeShiftSec=PeriodSeconds(InpInd_Timeframe);
//---- Инициализация переменных начала отсчета данных 
   min_rates_total=int(6*Length+3+1);
   ...
   if(!SignalBar || NB.IsNewBar(Symbol(),InpInd_Timeframe) || Recount) // проверка на появление нового бара
     {
      ...
      //if(!MQL5InfoInteger(MQL5_TESTING) && !MQL5InfoInteger(MQL5_OPTIMIZATION)) //если режим торговли в тестере "Произвольная задержка" 
        {
         if(SellPosOpen && SellPosClose  &&  TrendValue[1]==2) SELL_Close=true;
