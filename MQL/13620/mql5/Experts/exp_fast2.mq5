...
//+-----------------------------------------------+
//| Входные параметры для индикатора Fast2        |
//+-----------------------------------------------+
...
int OnInit()
  {
//---- получение хендла индикатора Fast2
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"Fast2",MA_Method1,Length1,Phase1,MA_Method2,Length2,Phase2,0);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print("Не удалось получить хендл индикатора Fast2");
      return(INIT_FAILED);
     }
   ...
   if(!SignalBar || NB.IsNewBar(Symbol(),InpInd_Timeframe) || Recount) // проверка на появление нового бара
     {
      ...
      if(CopyBuffer(InpInd_Handle,1,SignalBar,2,Ma1)<=0) {Recount=true; return;}
      if(CopyBuffer(InpInd_Handle,2,SignalBar,2,Ma2)<=0) {Recount=true; return;}
