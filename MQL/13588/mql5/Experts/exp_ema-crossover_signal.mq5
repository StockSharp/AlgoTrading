...
//+---------------------------------------------------+
//| Входные параметры индикатора EMA-Crossover_Signal |
//+---------------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H4; // Таймфрейм индикатора EMA-Crossover_Signal
...
int OnInit()
  {
//---- получение хендла индикатора EMA-Crossover_Signal
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"EMA-Crossover_Signal",FasterMA,SlowerMA,MAType,MAPrice);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора EMA-Crossover_Signal");
      return(INIT_FAILED);
     }
