...
#property description "Expert opens trades from the trend lines."
#property strict
...
//Trading conditions
//if(OrderExist() || MarketInfo(Symbol(),MODE_SPREAD)>slip || MarketInfo(Symbol(),MODE_MARGINREQUIRED)*lot>AccountEquity()) return;
   if(OrderExist() || MarketInfo(Symbol(),MODE_MARGINREQUIRED)*lot>AccountEquity()) return;
...
bool OrderExist()
  {
   ...
   for(int i=0; i<OrdersTotal(); i++)
     {
      if(OrderSelect(i,SELECT_BY_POS) && OrderSymbol()==Symbol() && OrderMagicNumber()==magic) return(true);
     }
   return(false);
  }
//+------------------------------------------------------------------+
...
void LevelStatus(int l)
  {
   if(!ObjectGet("Arrow_up"+DoubleToStr(l,0),OBJPROP_COLOR) && !ObjectGet("Arrow_down"+DoubleToStr(l,0),OBJPROP_COLOR) && !ObjectGet("Arrow_flat"+DoubleToStr(l,0),OBJPROP_COLOR))
     {
      //If all arrows were deleted set it againe and turn line "off"
      PlaceArrows(l);
