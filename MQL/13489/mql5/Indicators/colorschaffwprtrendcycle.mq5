//+------------------------------------------------------------------+
//|                                     ColorSchaffWPRTrendCycle.mq5 |
//|                                  Copyright © 2011, EarnForex.com |
//|                                        http://www.earnforex.com/ |
//+------------------------------------------------------------------+ 
#property copyright "Copyright © 2011, EarnForex.com"
#property link      "http://www.earnforex.com"
#property description "Schaff Trend Cycle - Cyclical Stoch over Stoch over WPR MACD."
...
#property indicator_label1 "Schaff WPR Trend Cycle"
//+-----------------------------------------------+
...
//+-----------------------------------------------+
input uint Fast_WPR = 23; // Период быстрого WPR
input uint Slow_WPR = 50; // Период медленного WPR
...
//---- инициализация переменных начала отсчета данных
   min_rates_1=int(Fast_WPR);
   min_rates_2=int(Slow_WPR);
   min_rates_total=MathMax(min_rates_1,min_rates_2)+int(Cycle)+1;
//--- получение хендла индикатора MA 1
   Ind1_Handle=iWPR(Symbol(),PERIOD_CURRENT,Fast_WPR);
   if(Ind1_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора WPR 1");
      return(INIT_FAILED);
     }
//--- получение хендла индикатора MA 2
   Ind2_Handle=iWPR(Symbol(),PERIOD_CURRENT,Slow_WPR);
   if(Ind2_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора WPR 2");
      return(INIT_FAILED);
     }
...
//---- инициализация переменной для короткого имени индикатора
   string shortname;
   StringConcatenate(shortname,"Schaff WPR Trend Cycle( ",Fast_WPR,", ",Slow_WPR,", ",Cycle," )");
...
//---- объявление переменных с плавающей точкой  
   double fastWPR[],slowWPR[],LLV,HHV;
...
//---- копируем вновь появившиеся данные в массивы
   if(CopyBuffer(Ind1_Handle,0,0,to_copy,fastWPR)<=0) return(RESET);
   if(CopyBuffer(Ind2_Handle,0,0,to_copy,slowWPR)<=0) return(RESET);
//---- индексация элементов в массивах как в таймсериях  
   ArraySetAsSeries(fastWPR,true);
   ArraySetAsSeries(slowWPR,true);
   ...
   for(bar=limit; bar>=0 && !IsStopped(); bar--)
     {
      ...
      Bar1=Count[1];
      //----
      MACD[Bar0]=fastWPR[bar]-slowWPR[bar];
      //----
