//+------------------------------------------------------------------+
//|                                 Exp_ColorSchaffWPRTrendCycle.mq5 |
...
//+------------------------------------------------------------+
//| Входные параметры индикатора ColorSchaffWPRTrendCycle      |
//+------------------------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H4; // Таймфрейм индикатора ColorSchaffWPRTrendCycle
...
int OnInit()
  {
//---- получение хендла индикатора ColorSchaffWPRTrendCycle
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"ColorSchaffWPRTrendCycle",Fast_WPR,Slow_WPR,Cycle,HighLevel,0,LowLevel);
   if(InpInd_Handle==INVALID_HANDLE)
    {
     Print(" Не удалось получить хендл индикатора ColorSchaffWPRTrendCycle");
     return(INIT_FAILED);
     }
//---- инициализация переменной для хранения периода графика в секундах  
   TimeShiftSec=PeriodSeconds(InpInd_Timeframe);
//---- инициализация переменных начала отсчета данных
   int min_rates_1=int(Fast_WPR);
   int min_rates_2=int(Slow_WPR);
