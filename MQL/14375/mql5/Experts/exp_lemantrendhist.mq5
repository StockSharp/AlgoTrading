...
//+----------------------------------------------+
//| Входные параметры индикатора LeManTrendHist  |
//+----------------------------------------------+
...
int OnInit()
  {
//---- получение хендла индикатора LeManTrendHist
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"LeManTrendHist",Min,Midle,Max,PeriodEMA,XMethod,XLength,XPhase,0);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора LeManTrendHist");
      return(INIT_FAILED);
     }
//---- инициализация переменной для хранения периода графика в секундах  
   TimeShiftSec=PeriodSeconds(InpInd_Timeframe);
//---- инициализация переменных начала отсчета данных
   min_rates_total=MathMax(MathMax(Min,Midle),Max)+1;
