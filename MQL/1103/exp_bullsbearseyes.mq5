...
//+----------------------------------------------+
//| BullsBearsEyes indicator input parameters    |
//+----------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H4; //indicator time frame
input int period_=13; //period of the indicator averaging
...
int OnInit()
  {
//---- getting handle of the ColorBullsBearsEyes indicator
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"ColorBullsBearsEyes",period_,gamma,Mode,HighLevel,MiddleLevel,LowLevel,0);
   if(InpInd_Handle==INVALID_HANDLE) Print(" Failed to get handle of ColorBullsBearsEyes indicator");

//---- initialization of a variable for storing a chart period in seconds  
   TimeShiftSec=PeriodSeconds(InpInd_Timeframe);

//---- Initialization of variables of the start of data calculation
   min_rates_total=int(period_+4);
   ...
   if(!SignalBar || NB.IsNewBar(Symbol(),InpInd_Timeframe) || Recount) // checking for a new bar
     {
      ...
      if(CopyBuffer(InpInd_Handle,2,SignalBar,1,Trend)<=0) {Recount=true; return;}

      //---- Getting buy signals
      if(Trend[0]>0)
        {
         if(BuyPosOpen&&Trend[0]==+2) BUY_Open=true;
      ...
      if(Trend[0]<0)
        {
         if(SellPosOpen&&Trend[0]==-2) SELL_Open=true;
