...
//+------------------------------------------------+
//| Входные параметры индикатора trend_arrows      |
//+------------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H1; //таймфрейм индикатора trend_arrows
...
int OnInit()
  {
//---- получение хендла индикатора trend_arrows
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"trend_arrows",iPeriod,iFullPeriods,0);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора trend_arrows");
      return(INIT_FAILED);
     }
