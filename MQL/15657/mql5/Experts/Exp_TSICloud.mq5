...
int OnInit()
  {
//---- получение хендла индикатора TSICloud
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"TSICloud",
                         First_Method,First_Length,First_Phase,Second_Method,Second_Length,Second_Phase,IPC,0,TriggerShift);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print("Не удалось получить хендл индикатора TSICloud");
      return(INIT_FAILED);
     }
...
//---- Инициализация переменных начала отсчёта данных
   ...
   min_rates_total+=GetStartBars(First_Method,First_Length,First_Phase);
   min_rates_total+=int(GetStartBars(Second_Method,Second_Length,Second_Phase)+TriggerShift);
