//+------------------------------------------------------------------+
//|                                     ColorSchaffRSITrendCycle.mq5 |
//|                                  Copyright © 2011, EarnForex.com |
//|                                        http://www.earnforex.com/ |
//+------------------------------------------------------------------+ 
#property copyright "Copyright © 2011, EarnForex.com"
#property link      "http://www.earnforex.com"
#property description "Schaff Trend Cycle - Cyclical Stoch over Stoch over RSI MACD."
...
#property indicator_label1 "Schaff RSI Trend Cycle"
//+-----------------------------------------------+
...
//+-----------------------------------------------+
input uint Fast_RSI = 23; // Период быстрого RSI
input uint Slow_RSI = 50; // Период медленного RSI
...
//---- инициализация переменных начала отсчета данных
   min_rates_1=int(Fast_RSI);
   min_rates_2=int(Slow_RSI);
   min_rates_total=MathMax(min_rates_1,min_rates_2)+int(Cycle)+1;
//--- получение хендла индикатора MA 1
   Ind1_Handle=iRSI(Symbol(),PERIOD_CURRENT,Fast_RSI,AppliedPrice);
...
//--- получение хендла индикатора MA 2
   Ind2_Handle=iRSI(Symbol(),PERIOD_CURRENT,Slow_RSI,AppliedPrice);
...
//---- инициализация переменной для короткого имени индикатора
   string shortname;
   StringConcatenate(shortname,"Schaff RSI Trend Cycle( ",Fast_RSI,", ",Slow_RSI,", ",Cycle," )");
...
//---- объявление переменных с плавающей точкой  
   double fastRSI[],slowRSI[],LLV,HHV;
...
//---- копируем вновь появившиеся данные в массивы
   if(CopyBuffer(Ind1_Handle,0,0,to_copy,fastRSI)<=0) return(RESET);
   if(CopyBuffer(Ind2_Handle,0,0,to_copy,slowRSI)<=0) return(RESET);
//---- индексация элементов в массивах как в таймсериях  
   ArraySetAsSeries(fastRSI,true);
   ArraySetAsSeries(slowRSI,true);
   ...
   for(bar=limit; bar>=0 && !IsStopped(); bar--)
     {
      ...
      Bar1=Count[1];
      //----
      MACD[Bar0]=fastRSI[bar]-slowRSI[bar];
      //----
