...
//----
input int Length=8; // period of Momentum
...
int OnInit()
  {
//---- getting the ColorMomentum_AMA indicator handle
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"ColorMomentum_AMA",Length,ama_period,fast_ma_period,slow_ma_period,IPC,G,0);
   if(InpInd_Handle==INVALID_HANDLE) Print(" Failed to get handle of ColorMomentum_AMA indicator");

//---- initialization of a variable for storing a chart period in seconds  
   TimeShiftSec=PeriodSeconds(InpInd_Timeframe);

//---- Initialization of variables of the start of data calculation
   min_rates_total=Length+ama_period+2;
   ...
   if(!SignalBar || NB.IsNewBar(Symbol(),InpInd_Timeframe) || Recount) // checking for a new bar
     {
      ...
      if(CopyBuffer(InpInd_Handle,4,SignalBar,3,Value)<=0) {Recount=true; return;}
