...
//+------------------------------------------------+
//| Входные параметры индикатора NRTR              |
//+------------------------------------------------+
input ENUM_TIMEFRAMES InpInd_Timeframe=PERIOD_H1; //таймфрейм индикатора NRTR
...
int OnInit()
  {
//---- получение хендла индикатора NRTR
   InpInd_Handle=iCustom(Symbol(),InpInd_Timeframe,"NRTR",iPeriod,iDig,0);
   if(InpInd_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора NRTR");
      return(INIT_FAILED);
     }
