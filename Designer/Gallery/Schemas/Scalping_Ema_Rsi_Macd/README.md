...
- **Exit**: The position protection block closes every trade on a percentage stop or target measured from the fill price. These are fixed percentage distances; using ATR-based distances instead would require calculating the levels in the diagram and sending the orders individually. The stop or target ends the trade, so an opposite signal does not reverse an open position. The diagram runs on the five-minute candles of the packaged history.
