...
- **Saída**: O bloco de proteção de posição encerra cada operação com um stop ou um alvo percentual medidos a partir do preço de execução. Essas são distâncias percentuais fixas; usar distâncias baseadas em ATR exigiria calcular os níveis no diagrama e enviar as ordens individualmente. O stop ou o alvo encerra a operação, portanto um sinal contrário não inverte uma posição aberta. O diagrama trabalha com os candles de cinco minutos do histórico empacotado.
