...
- **エグジット**: 平均線の下へ引け直せば買い建てを、上へ引け直せば売り建てを閉じます。どちらもクローズモードのポジション変更ブロックです。ポジションがゼロのときクローズブロックは何もしないため、追加のクロス判定ブロックは不要です。同じ曜日の窓の中で価格が平均線の必要な側へ戻れば、すぐに再エントリーできます。
