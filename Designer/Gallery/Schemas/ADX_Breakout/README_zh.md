...
- 公式方块把带构造为：均线加上乘数乘以 ADX 与其均线绝对距离的两倍。
...
- **离场**: 只要 ADX 线回落到自己的移动平均之下就平仓：多头用平仓模式的卖单，空头用平仓模式的买单。此外，持仓保护方块设有 2% 的止损；止盈已关闭，因此没有连接目标位。优化之前值得知道一点：只要乘数小于 0.5，带条件在代数上就等同于“ADX 高于其均线”，所以在默认值 0.1 下带没有起任何作用，图可以直接读作 ADX 上穿与下穿自身均线。乘数仍保留为常量，取较大值时可以让带条件更加严格。
...
- 一个公式方块用单一表达式算出整条带，把算术集中在一个可读之处，而不是拆成一串小方块。
