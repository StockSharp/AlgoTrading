...
- The simple moving average takes no part in the entry; it is only the line the trade is given up on.
...
- **Exit**: A long is closed once a candle closes below the moving average, a short once a candle closes above it, both through position modify blocks in close mode. The diagram has neither a stop loss nor a take profit and takes every qualifying pattern it sees.
...
| Candles | 00:05:00 | Five-minute candle time frame used to match the packaged history and keep the pattern readable. |
