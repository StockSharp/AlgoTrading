...
- The %K line is calculated on finished candles of a single instrument; the smoothed %D line takes no part in the decision.
...
- The oversold and overbought levels are editable and optimizable constants of the diagram, set to 20 and 80 by default.
...
- **Exit**: There is no separate exit block: the opposite level crossing closes the position, because every order carries the same volume. Crossing rather than merely remaining inside a zone keeps the diagram from firing on every candle.
