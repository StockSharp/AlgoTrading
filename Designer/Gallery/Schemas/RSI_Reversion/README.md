...
- A SimpleMovingAverage of 50 candles never picks a side; it only holds trading back until it is formed.
...
- **Exit**: There is no separate exit block, stop loss or take profit: the opposite reversion signal closes the position and opens the other side with the same order.
...
- Two comparison blocks per side test the previous and the current reading against the level constant, forming the crossing condition.
- The comparison of the SMA against zero acts as a readiness guard; since the indicator block emits formed values only, trading starts once fifty candles are in.
