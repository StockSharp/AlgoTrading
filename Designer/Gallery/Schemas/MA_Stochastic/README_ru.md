...
- StochasticK вычисляет %K как 100 * (Close - минимум Low) / (максимум High - минимум Low) за последние N свечей.
...
- Сравнения по тренду переиспользуются на выходе: тот же сигнал, что разрешает шорт, закрывает лонг, — за счёт этого схема остаётся компактной.
