...
The Hull Moving Average follows price with very little lag, so the direction of its own slope is already a trend signal. The diagram measures how far the average moved since the previous candle, as a fraction of its own value, and turns the position to that side once the move is larger than a small threshold. Its length is 100 five-minute candles, a 500-minute span on the packaged history.
...
- After the first signal the strategy is always in the market: there is no stop, no target and no flat state between trades.
...
|---|---|---|
| Hull MA Length | 100 | Length of the Hull Moving Average on five-minute candles. |
...
- A previous-value block keeps the Hull average of the previous candle and stays silent on the first value, so the first bar is skipped.
