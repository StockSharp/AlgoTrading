...
- 注文数量は基準数量にポジションの絶対値を加えた値なので、1回の注文で売り建てをそのまま買い建てへ、あるいはその逆へ転換できます。
- 平均足の始値は通常その前の値で定義されますが、ダイアグラムではブロックの出力を自分自身へ戻せません。そこで前の通常足の中値で代用し、連続を近似します。
...
- **エグジット**: 専用のシグナルエグジットはありません。ポジションは反対の連続で転換されるか、ポジション保護ブロックの損切りで外れます。損切りは約定価格から一定の割合の位置に置かれ、利食いもトレーリングもありません。
...
| Candles | 00:30:00 | ダイアグラム全体で使用する30分足の時間軸。 |
