...
DeMarker measures how far each candle reaches beyond the previous one, upwards against downwards, and returns a value between 0 and 1. This diagram does not buy the extreme, it buys the way back from it: a reading that climbs from below the oversold level up to it buys, a reading that falls from above the overbought level back to it sells. It uses five-minute candles, and the position guard blocks a second entry in the same direction.
