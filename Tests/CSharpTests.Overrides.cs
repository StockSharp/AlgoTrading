...
partial class CSharpTests
{
	...
	{
		const string path = "0901-1000/0902_Hurst_Future_Lines_of_Demarcation/CS/HurstFutureLinesOfDemarcationStrategy.cs";
		string[] ids = null;
	...
	{
		const string path = "1201-1300/1209_Quadratic_Regression/CS/QuadraticRegressionStrategy.cs";
		var recorder = new OrderTraceRecorder();
		string[] ids = null;
		...
		{
			ids = strategy.Parameters.CachedKeys;
			strategy.Parameters["Length"].Value = 1000000;
			recorder.Attach(strategy);
		}, requireTrades: false);
	...
	public async Task S1801_PerceptronContract()
	{
		...
		await RunStrategy(path, CancellationToken, (strategy, _) =>
		{
			...
			foreach (var name in new[] { "X1", "X2", "X3", "X4" })
				strategy.Parameters[name].Value = 0m;
			recorder.Attach(strategy);
		}, requireTrades: false);
	...
	public async Task S2907_CcfpCurrencyStrengthContract()
	{
		...
		await RunStrategy<CcfpCurrencyStrengthStrategy>(CancellationToken, (strategy, second) =>
		{
			var firstId = strategy.Security.Id;
			var secondId = second.Id;
			strategy.EURUSD = firstId;
			strategy.GBPUSD = secondId;
			strategy.AUDUSD = firstId;
			strategy.NZDUSD = secondId;
			strategy.USDCAD = firstId;
			strategy.USDCHF = secondId;
			strategy.USDJPY = firstId;
			ids = strategy.Parameters.CachedKeys;
		}, requireTrades: false);
	...
	{
		string[] ids = null;
		await RunStrategy<MultiTimeframeMacdStrategy>(CancellationToken, (strategy, _) =>
		{
			strategy.CandleType = TimeSpan.FromMinutes(5).TimeFrame();
			strategy.HigherCandleType = TimeSpan.FromMinutes(15).TimeFrame();
			ids = strategy.Parameters.CachedKeys;
		}, requireTrades: false);
	...
	{
		string[] ids = null;
		await RunStrategy<HedgeAverageStrategy>(CancellationToken, (strategy, _) => ids = strategy.Parameters.CachedKeys, requireTrades: false);
