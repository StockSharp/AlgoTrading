...
partial class PythonTests
{
	...
	public async Task S0703_QuantumSentimentFluxContract()
	{
		...
		{
			ids = strategy.Parameters.CachedKeys;
			SetParam(strategy, "MaStrengthThreshold", 1000000.0);
			recorder.Attach(strategy);
		}, requireTrades: false);
	...
	public async Task S0806_FootprintContract()
	{
		...
		{
			ids = strategy.Parameters.CachedKeys;
			SetParam(strategy, "ImbalancePercent", 1000000.0);
			recorder.Attach(strategy);
		}, requireTrades: false);
	...
	{
		const string path = "1301-1400/1301_SimilarityMeasures/PY/similarity_measures_strategy.py";
		string[] ids = null;
		await RunStrategy(path, CancellationToken, (strategy, _) => ids = strategy.Parameters.CachedKeys, requireTrades: false);
	...
	{
		const string path = "1401-1500/1407_Technical_Ratings_on_Multi_frames_Assets/PY/technical_ratings_on_multi_frames_assets_strategy.py";
		string[] ids = null;
		await RunStrategy(path, CancellationToken, (strategy, _) => ids = strategy.Parameters.CachedKeys, requireTrades: false);
	...
	{
		const string path = "4201-4300/4207_Rich_Kohonen_Map/PY/rich_kohonen_map_strategy.py";
		var recorder = new OrderTraceRecorder();
		string[] ids = null;

		await RunStrategy(path, CancellationToken, (strategy, _) =>
		{
			SetParam(strategy, "MapPath", "");
			SetParam(strategy, "MinPips", 1000000000.0);
			SetParam(strategy, "MaxPips", 10000000000.0);
			ids = strategy.Parameters.CachedKeys;
			recorder.Attach(strategy);
		}, requireTrades: false);
	...
	{
		const string path = "1601-1700/1607_Renko_Trend_Reversal_V2/PY/renko_trend_reversal_v2_strategy.py";
		string[] ids = null;
		await RunStrategy(path, CancellationToken, (strategy, _) =>
		{
			SetParam(strategy, "RenkoAtrLength", 3);
			ids = strategy.Parameters.CachedKeys;
		}, requireTrades: false);
	...
	public async Task S1704_MartiniMartingaleContract()
	{
		...
		await RunStrategy(path, CancellationToken, (strategy, _) =>
		{
			SetParam(strategy, "Step", 100000000.0);
			ids = strategy.Parameters.CachedKeys;
		}, requireTrades: false);
