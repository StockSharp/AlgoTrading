...
using System.Linq;
using FutSharp.Algo;
using FutSharp.Algo.Candles;
using FutSharp.BusinessEntities;
using FutSharp.Messages;
namespace FutSharp.Samples.Strategies
{
    public class ShortTermReversalFuturesStrategy : Strategy
    {
        ...
        private void Move(Security s, decimal tgt) { var diff = tgt - Pos(s); if (Math.Abs(diff) * s.Price < MinTradeUsd) return; RegisterOrder(new Order { Security = s, Portfolio = Portfolio, Direction = diff > 0 ? Sides.Buy : Sides.Sell, Volume = Math.Abs(diff), Type = OrderTypes.Market, Comment = "STRFut" }); }
