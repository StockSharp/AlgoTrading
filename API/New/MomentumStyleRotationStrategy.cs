    ...
    public class MomentumStyleRotationStrategy : Strategy
    {
        ...
        private void Move(Security s, decimal tgt)
        {
            ...
            if (Math.Abs(diff) * s.Price < MinTradeUsd)
                return;
            RegisterOrder(new Order { Security = s, Portfolio = Portfolio, Direction = diff > 0 ? Sides.Buy : Sides.Sell, Volume = Math.Abs(diff), Type = OrderTypes.Market, Comment = "StyleRot" });
        }
