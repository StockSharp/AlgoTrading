...
namespace StockSharp.Samples.Strategies
{
    public class WTIBrentSpreadStrategy : Strategy
    {
        ...
        public override IEnumerable<(Security, DataType)> GetWorkingSecurities() { yield return (WTI, _tf); yield return (Brent, _tf); }
        protected override void OnStarted(DateTimeOffset t)
        {
            base.OnStarted(t);
            SubscribeCandles(WTI, _tf).Bind(CandleStates.Finished).Do(c => OnDaily()).Start();
        }
        private void OnDaily()
        {
            ...
            { Move(WTI, 0); Move(Brent, 0); return; }
            if (spr > sma)
            { Move(WTI, -notional / p1); Move(Brent, notional / p2); }
            else
            { Move(WTI, notional / p1); Move(Brent, -notional / p2); }
        }
