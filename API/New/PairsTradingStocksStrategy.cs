...
namespace StockSharp.Samples.Strategies
{
    public class PairsTradingStocksStrategy : Strategy
    {
        ...
        protected override void OnStarted(DateTimeOffset t)
        {
            ...
            {
                _hist[(a, b)] = new Win();
                SubscribeCandles(a, _tf).Bind(CandleStates.Finished).Do(c => OnDaily()).Start();
            }
        }
        ...
        private void Move(Security s, decimal tgt)
        {
            ...
            if (Math.Abs(diff) * s.Price < MinTradeUsd)
                return;
            RegisterOrder(new Order { Security = s, Portfolio = Portfolio, Direction = diff > 0 ? Sides.Buy : Sides.Sell, Volume = Math.Abs(diff), Type = OrderTypes.Market, Comment = "Pairs" });
        }
