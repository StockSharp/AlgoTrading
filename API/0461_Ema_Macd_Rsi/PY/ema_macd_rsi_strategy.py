...
from StockSharp.Algo.Indicators import ExponentialMovingAverage, MovingAverageConvergenceDivergenceSignal, RelativeStrengthIndex
...
class ema_macd_rsi_strategy(Strategy):
    ...
    def OnStarted(self, time):
        super(ema_macd_rsi_strategy, self).OnStarted(time)
        ...
        subscription \
            .BindEx(self._fast_ema, self._slow_ema, self._macd_signal, self._rsi, self.OnProcess) \
            .Start()
    ...
    def OnProcess(self, candle, fast_ema_val, slow_ema_val, macd_val, rsi_val):
        ...
        fast_ema = float(fast_ema_val)
        slow_ema = float(slow_ema_val)
        rsi = float(rsi_val)
        macd = float(macd)
        signal = float(signal)
        ...
        rsi_bullish = rsi > float(self.rsi_buy_level) and rsi < 70.0
        rsi_bearish = rsi < float(self.rsi_sell_level) and rsi > 30.0
