...
class larry_connors_3_day_high_low_strategy(Strategy):
    ...
    def OnStarted(self, time):
        super(larry_connors_3_day_high_low_strategy, self).OnStarted(time)
        self._entries_executed = 0
        self._bars_since_signal = self._cooldown_bars.Value
        long_sma = SimpleMovingAverage()
        long_sma.Length = self._long_ma_length.Value
        short_sma = SimpleMovingAverage()
        short_sma.Length = self._short_ma_length.Value
        subscription = self.SubscribeCandles(self.candle_type)
        subscription.Bind(long_sma, short_sma, self.OnProcess).Start()
    ...
    def OnProcess(self, candle, long_ma_val, short_ma_val):
        ...
        if self._bar_count >= 3 and self._bars_since_signal >= self._cooldown_bars.Value:
            cond1 = close > lma
            cond2 = close < sma
            cond3 = self._high2 < self._high3 and self._low2 < self._low3
            cond4 = self._high1 < self._high2 and self._low1 < self._low2
            cond5 = high < self._high1 and low < self._low1
            if cond1 and cond2 and cond3 and cond4 and cond5 and self.Position <= 0 and self._entries_executed < self._max_entries.Value:
                self.BuyMarket()
