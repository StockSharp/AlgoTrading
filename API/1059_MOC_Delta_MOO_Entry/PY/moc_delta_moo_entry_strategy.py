...
class moc_delta_moo_entry_strategy(Strategy):
    def __init__(self):
        super(moc_delta_moo_entry_strategy, self).__init__()
        self._candle_type = self.Param("CandleType", DataType.TimeFrame(TimeSpan.FromMinutes(15))) \
            .SetDisplay("Candle Type", "Candles timeframe", "General")
        self._delta_window = self.Param("DeltaWindow", 20) \
            .SetGreaterThanZero() \
            .SetDisplay("Delta Window", "Bars per delta calculation window", "General")
        self._delta_threshold_percent = self.Param("DeltaThresholdPercent", 8.0) \
            .SetGreaterThanZero() \
    ...
    def OnReseted(self):
        super(moc_delta_moo_entry_strategy, self).OnReseted()
        self._window_buy_volume = 0.0
        self._window_sell_volume = 0.0
        self._window_bar_count = 0
        self._bars_from_signal = 0
    ...
    def OnStarted2(self, time):
        super(moc_delta_moo_entry_strategy, self).OnStarted2(time)
        self._window_buy_volume = 0.0
        self._window_sell_volume = 0.0
        self._window_bar_count = 0
    ...
    def OnProcess(self, candle):
        ...
        if delta_percent > thr:
            ...
            momentum_signal = -1
        if self._bars_from_signal >= self._signal_cooldown_bars.Value and momentum_signal != 0:
            if momentum_signal > 0 and self.Position <= 0:
                self.BuyMarket()
                self._bars_from_signal = 0
            elif momentum_signal < 0 and self.Position >= 0:
                self.SellMarket()
                self._bars_from_signal = 0
        self._window_buy_volume = 0.0
        self._window_sell_volume = 0.0
    ...
    def CreateClone(self):
        return moc_delta_moo_entry_strategy()
