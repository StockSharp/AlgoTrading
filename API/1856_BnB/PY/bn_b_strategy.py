...
class bn_b_strategy(Strategy):
    def __init__(self):
        super(bn_b_strategy, self).__init__()
    ...
    def OnReseted(self):
        super(bn_b_strategy, self).OnReseted()
        self._prev_bull = 0.0
        self._prev_bear = 0.0
        self._initialized = False
        self._bull_ema = 0.0
    ...
    def OnStarted(self, time):
        super(bn_b_strategy, self).OnStarted(time)
        self._k = 2.0 / (self.length + 1.0)
        self._count = 0
        sma = SimpleMovingAverage()
        sma.Length = self.length
        sub = self.SubscribeCandles(self.candle_type)
        sub.Bind(sma, self.process_candle).Start()
    ...
    def process_candle(self, candle, sma_value):
        ...
        bull_power = float(candle.HighPrice) - float(sma_value)
        bear_power = float(candle.LowPrice) - float(sma_value)
        self._count += 1
        ...
        cross_up = prev_net <= 0.0 and net_power > 0.0 and abs(net_power) >= float(self.min_net_power)
        cross_down = prev_net >= 0.0 and net_power < 0.0 and abs(net_power) >= float(self.min_net_power)
