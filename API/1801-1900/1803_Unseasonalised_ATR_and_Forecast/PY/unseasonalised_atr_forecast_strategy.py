...
class unseasonalised_atr_forecast_strategy(Strategy):
    def __init__(self):
        super(unseasonalised_atr_forecast_strategy, self).__init__()
        self._fast_period = self.Param("FastPeriod", 12)             .SetDisplay("Fast Period", "Fast EMA period", "Indicators")
        self._slow_period = self.Param("SlowPeriod", 36)             .SetDisplay("Slow Period", "Slow EMA period", "Parameters")
    ...
    def OnReseted(self):
        super(unseasonalised_atr_forecast_strategy, self).OnReseted()
        self._prev_fast = 0.0
        self._prev_slow = 0.0
        self._has_prev = False

    def OnStarted2(self, time):
        super(unseasonalised_atr_forecast_strategy, self).OnStarted2(time)
    ...
    def CreateClone(self):
        return unseasonalised_atr_forecast_strategy()
