...
/// The strategy keeps a single net position and emulates the expert advisor's behaviour within that constraint.
/// </summary>
...
_trailingStepPoints = Param(nameof(TrailingStepPoints), 0m)
		.SetNotNegative()
	...
	private void ProcessCandle(ICandleMessage candle, decimal cciValue, decimal impulseValue)
	{
		...
		{
			// Capture the very first complete values before taking any decision.
		...
		{
			// Wait until both oscillators have at least one historical value.
			_previousCci = cciValue;
			_previousImpulse = impulseValue;
			_hasPreviousCci = true;
			_hasPreviousImpulse = true;
		...
		{
			// The original MQL script throttles entries via a global variable; emulate the same safety gap.
			_previousCci = cciValue;
			_previousImpulse = impulseValue;
			return;
		}
		...
		if (Position != 0)
		{
			// Manage the position using protective exits and trailing logic.
	...
	private void HandleActivePosition(ICandleMessage candle, DateTimeOffset now)
	{
		...
		if (Position > 0)
		{
			// Track the best excursion to align the trailing stop with price advances.
		...
		{
			// Track the lowest price to mirror the trailing logic for short trades.
	...
	private void UpdateZigZag(ICandleMessage candle)
	{
		...
		{
			// Initialise the very first extreme according to the current search direction.
	...
	private sealed class ImpulseIndicator : Indicator<ICandleMessage>
	{
		...
		protected override IIndicatorValue OnProcess(IIndicatorValue input)
		{
			...
			// Approximate the original DayImpuls buffers by averaging the open-close distance.
