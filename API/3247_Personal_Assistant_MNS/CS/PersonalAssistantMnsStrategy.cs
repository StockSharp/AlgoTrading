...
/// <summary>Trading volume for manual market orders.</summary>
...
/// <summary>Maximum tolerated slippage in ticks (informational only).</summary>
public int Slippage
{
get => _slippage.Value;
...
.SetGreaterThanZero()
.SetDisplay("Volume", "Volume for manual trades", "Trading");

_slippage = Param(nameof(Slippage), 2)
.SetDisplay("Slippage", "Maximum tolerated slippage in ticks", "Trading");
...
if (isBuy)
{
price = Security?.BestAskPrice ?? Security?.LastTrade?.Price ?? Security?.LastPrice;
}
else
{
price = Security?.BestBidPrice ?? Security?.LastTrade?.Price ?? Security?.LastPrice;
}
