...
public class CdcPlMfiStrategy : Strategy
{
	...
	private void ProcessCandle(ICandleMessage candle, decimal mfiValue)
	{
		...
		if (_candles.Count >= 2 && _hasPrevMfi)
		{
			...
			if (isPiercing && mfiValue < LongLevel && Position <= 0)
				BuyMarket();
			else if (isDarkCloud && mfiValue > ShortLevel && Position >= 0)
				SellMarket();

			// Exit on MFI crossing
			if (Position > 0 && _prevMfi >= ShortLevel && mfiValue < ShortLevel)
				SellMarket();
			else if (Position < 0 && _prevMfi <= LongLevel && mfiValue > LongLevel)
				BuyMarket();
		}
