...
/// </summary>
public class MacdPatternTraderAdvancedStrategy : Strategy
{
	...
	/// Constructor.
	/// </summary>
	public MacdPatternTraderAdvancedStrategy()
	{
	...
	protected override void OnStarted(DateTimeOffset time)
	{
		...
		// Create a candle subscription and pipe indicator values into a single handler.
	...
	{
		// Process only completed candles to avoid double counting intrabar states.
		if (candle.State != CandleStates.Finished)
			return;

		_candles.Add(candle);
		// Maintain a rolling candle window for stop-loss and take-profit scans.
		TrimCandles();

		// Update trailing stops and partial exits before evaluating new entries.
		CheckRiskManagement(candle);

		// Wait until every indicator reports a final value for the finished bar.
		...
		// Verify that the MACD indicators delivered composite values with MACD components.
		...
		// Extract the MACD main line for each configured pattern.
	...
	// Pattern #1: detect MACD reversals beyond symmetric thresholds.
	...
	// Pattern #2: trade zero-line bounces from positive or negative phases.
	...
	// Pattern #3: reproduce the tiered crest and trough recognition with staged flags.
	...
	// Pattern #4: identify local MACD peaks and valleys before reversing.
	...
	// Pattern #5: monitor neutral MACD bands and react to immediate breakouts.
	...
	// Pattern #6: count consecutive threshold breaches to schedule trades.
	...
	// Attempt to open a long position and configure protective levels.
	...
	// Attempt to open a short position mirroring the long setup logic.
	...
	// Handle exits, stop calculations and martingale resets prior to new entries.
	...
	// Record MACD values for unfinished candles so the original counters stay consistent.
	...
	// Reset or double the working volume according to martingale rules once flat.
