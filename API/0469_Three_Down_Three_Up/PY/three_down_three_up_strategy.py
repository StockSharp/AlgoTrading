...
class three_down_three_up_strategy(Strategy):
    ...
    def __init__(self):
        ...
        self._buy_trigger = self.Param("BuyTrigger", 3).SetGreaterThanZero().SetDisplay("Buy Trigger", "Consecutive down closes for entry", "Trading")
        self._sell_trigger = self.Param("SellTrigger", 3).SetGreaterThanZero().SetDisplay("Sell Trigger", "Consecutive up closes for exit", "Trading")
    ...
    def OnProcess(self, candle, ema_val):
        ...
        # Buy after consecutive down closes
        if self._down_count >= buy_trigger and self.Position <= 0:
            if self.Position < 0:
                self.BuyMarket()
            self.BuyMarket()
            self._up_count = 0
            self._cooldown_remaining = self._cooldown_bars.Value
        # Sell after consecutive up closes
        elif self._up_count >= sell_trigger and self.Position >= 0:
            if self.Position > 0:
                self.SellMarket()
            self.SellMarket()
            self._down_count = 0
        ...
        elif self.Position > 0 and close < ema_val and self._up_count >= 2:
            self.SellMarket()
            self._cooldown_remaining = self._cooldown_bars.Value
        # Exit short: price above EMA + down streak
        elif self.Position < 0 and close > ema_val and self._down_count >= 2:
            self.BuyMarket()
            self._cooldown_remaining = self._cooldown_bars.Value
