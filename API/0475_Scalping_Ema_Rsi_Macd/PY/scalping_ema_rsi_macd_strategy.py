...
from StockSharp.Algo.Indicators import ExponentialMovingAverage, RelativeStrengthIndex, MovingAverageConvergenceDivergence, AverageTrueRange
...
class scalping_ema_rsi_macd_strategy(Strategy):
    ...
    def OnStarted(self, time):
        super(scalping_ema_rsi_macd_strategy, self).OnStarted(time)
        ...
        macd.ShortMa.Length = self._fast_ema_length.Value
        macd.LongMa.Length = self._slow_ema_length.Value
        atr = AverageTrueRange()
        atr.Length = self._atr_length.Value
        subscription = self.SubscribeCandles(self.candle_type)
        subscription.Bind(fast_ema, slow_ema, trend_ema, rsi, macd, atr, self.OnProcess).Start()
    ...
    def OnProcess(self, candle, fast_ema, slow_ema, trend_ema, rsi, macd, atr):
