...
class warrior_trading_momentum_strategy(Strategy):
    def __init__(self):
        ...
        self._vol_mult = self.Param("VolMult", TimeSpan.FromHours(4)) \
    ...
    def on_process(self, candle, ema_val, rsi_val, std_val):
        ...
        while (len(self._volumes) > self.vol_avg_length + 1)
        self._volumes.pop(0)
        # TP/SL exit
        if self.Position > 0:
        ...
        if candle.ClosePrice < candle.OpenPrice:
            ...
            self._red_count = 0
        if std_val <= 0 or len(self._volumes) < self.vol_avg_length or self._daily_trades >= self.max_daily_trades:
            return
        if self.Position != 0:
            return
        vol_avg = self._volumes.Take(self.vol_avg_length).Sum() / self.vol_avg_length
        volume_spike = candle.TotalVolume > vol_avg * self.vol_mult
        bullish = candle.ClosePrice > candle.OpenPrice
        # Red-to-green reversal with volume spike
        if self._red_count >= self.min_red_candles and bullish and volume_spike and candle.ClosePrice > ema_val and rsi_val > 55:
            self.BuyMarket()
            stop_dist = std_val * 2
            self._stop_price = candle.ClosePrice - stop_dist
            self._take_profit_price = candle.ClosePrice + stop_dist * self.risk_reward
            self._daily_trades += 1
        elif candle.ClosePrice < ema_val and rsi_val < 45 and volume_spike:
            self.SellMarket()
            stop_dist = std_val * 2
            self._stop_price = candle.ClosePrice + stop_dist
            self._take_profit_price = candle.ClosePrice - stop_dist * self.risk_reward
