...
该策略基于 MetaTrader 的 “RenkoLineBreak vs RSI” 专家顾问，使用 StockSharp 的高级 API 重新实现。策略利用 Renko 砖块判断趋势方向，同时通过 RSI 回调过滤信号，并围绕三根 K 线结构挂出止损挂单。Renko 砖块由策略内部根据时间 K 线的收盘价计算，因此整个策略只依赖一条 K 线订阅。
...
1. Renko 砖块在策略内部由时间 K 线的收盘价生成：当收盘价相对当前基准价移动满一个 `BoxSize` 时，就产生一块或多块砖，基准价随之推移。砖块翻转时，趋势状态会暂时变为 `ToUp` 或 `ToDown`，模拟原指标的信号。
