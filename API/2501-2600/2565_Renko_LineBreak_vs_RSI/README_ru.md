...
Стратегия повторяет логику советника «RenkoLineBreak vs RSI» из MetaTrader с использованием высокоуровневого API StockSharp. Для определения направления используются кирпичи Renko, фильтрация входов выполняется по RSI, а заявки выставляются в виде отложенных стопов на основе структуры из трёх свечей. Кирпичи Renko рассчитываются внутри самой стратегии по ценам закрытия временных свечей, поэтому всю работу обеспечивает одна подписка на свечи.
...
1. Кирпичи Renko строятся внутри стратегии по ценам закрытия временных свечей: как только закрытие уходит от текущей опорной точки на полный `BoxSize`, формируется один или несколько кирпичей, а опорная точка сдвигается вслед за ними. При смене направления кирпича тренд переводится в состояние `ToUp` или `ToDown`, что имитирует оригинальный индикатор.
