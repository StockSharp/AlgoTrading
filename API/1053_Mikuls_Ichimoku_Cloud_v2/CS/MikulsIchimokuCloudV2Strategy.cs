...
public class MikulsIchimokuCloudV2Strategy : Strategy
{
	...
	public MikulsIchimokuCloudV2Strategy()
	{
		...
		_atrMultiplier = Param(nameof(AtrMultiplier), 1m).SetGreaterThanZero();
		_tenkanPeriod = Param(nameof(TenkanPeriod), 9).SetGreaterThanZero();
		_kijunPeriod = Param(nameof(KijunPeriod), 26).SetGreaterThanZero();
		_senkouBPeriod = Param(nameof(SenkouBPeriod), 52).SetGreaterThanZero();
	...
	private void ProcessCandle(ICandleMessage candle, IIndicatorValue ichimokuValue, IIndicatorValue atrValue)
	{
		...
		var pamp = close > upperCloud && close > open && senkouA > senkouB && tenkan > kijun;
		var trend = crossUp && close > upperCloud && close > open && tenkan > upperCloud && kijun > upperCloud;
		...
		// Trailing stop using ATR + Ichimoku exit
		if (Position > 0)
		{
			...
			else if (tenkan < kijun || close < lowerCloud)
			{
				SellMarket();
				_trailPrice = null;
			}
