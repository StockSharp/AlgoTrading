...
/// <summary>
/// Martingale-style averaging strategy converted from the MetaTrader "10 points 3" expert advisor.
/// </summary>
public class TenPoints3Strategy : Strategy
{
	...
	/// <summary>
	/// Initializes a new instance of <see cref="TenPoints3Strategy"/>.
	/// </summary>
	public TenPoints3Strategy()
	{
		...
		_lotSize = Param(nameof(LotSize), 1.2m)
			.SetDisplay("Base Lot Size", "Fixed lot size used when money management is disabled", "Risk")
			.SetCanOptimize(true);

		_initialStopPips = Param(nameof(InitialStopPips), 110m)
		...
		_ordersToProtect = Param(nameof(OrdersToProtect), 6)
			.SetGreaterThanZero()
		...
		_eurUsdPipValue = Param(nameof(EurUsdPipValue), 40m)
		...
		_endYear = Param(nameof(EndYear), 2009)
