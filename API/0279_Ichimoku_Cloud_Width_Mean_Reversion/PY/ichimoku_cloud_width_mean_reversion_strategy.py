...
class ichimoku_cloud_width_mean_reversion_strategy(Strategy):
    ...
    def ProcessIchimoku(self, candle, ichimokuValue):
        ...
        cloudWidthAverage = float(process_float(self._cloudWidthAverage, self._currentCloudWidth, candle.ServerTime, candle.State == CandleStates.Finished))
        cloudWidthStdDev = float(process_float(self._cloudWidthStdDev, self._currentCloudWidth, candle.ServerTime, candle.State == CandleStates.Finished))
