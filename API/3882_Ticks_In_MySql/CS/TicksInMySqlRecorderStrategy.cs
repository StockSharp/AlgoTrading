...
/// <summary>
/// Strategy that streams account metrics and quotes into a MySQL table for every level 1 update.
/// Mirrors the original MetaTrader expert that inserted a row on every tick.
/// </summary>
public class TicksInMySqlRecorderStrategy : Strategy
{
	...
	private readonly StrategyParam<bool> _ensureTableExists;
	private readonly StrategyParam<int> _pricePrecision;
	private readonly StrategyParam<bool> _stopOnInsertError;
	...
	/// </summary>
	public TicksInMySqlRecorderStrategy()
	{
		...
		_ensureTableExists = Param(nameof(EnsureTableExists), false)
		...
		_stopOnInsertError = Param(nameof(StopOnInsertError), false)
			.SetDisplay("Stop On Insert Error", "Stop the strategy when a database insert fails", "Connection");
	}
	...
	/// <summary>
	/// Automatically create the table when it is missing.
	/// </summary>
	public bool EnsureTableExists
	{
		get => _ensureTableExists.Value;
		set => _ensureTableExists.Value = value;
	}

	/// <summary>
	/// Digits stored for bid/ask.
	/// </summary>
	...
	/// <summary>
	/// Stop the strategy when an insert fails.
	/// </summary>
	public bool StopOnInsertError
	{
		get => _stopOnInsertError.Value;
		set => _stopOnInsertError.Value = value;
	}
	...
	{
		// Cache the latest bid/ask prices to mimic the tick updates from MetaTrader.
		...
		// Gather the account snapshot that mirrors AccountMargin, AccountFreeMargin, and AccountEquity from MQL.
