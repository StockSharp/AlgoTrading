	...
	public class CciSlopeMeanReversionStrategy : Strategy
	{
		...
		public CciSlopeMeanReversionStrategy()
		{
			_cciPeriod = Param(nameof(CciPeriod), 20)
				.SetDisplayName("CCI Period")
				.SetDescription("Commodity Channel Index period")
				.SetCategory("CCI Settings")
				.SetCanOptimize(true)
				.SetOptimize(10, 40, 5);
			...
			if (_previousCciValue != 0)
			{
				...
				if (_slopeCount >= SlopeLookback)
				{
					...
					if (_currentSlope < lowerThreshold && Position <= 0)
					{
						...
						this.AddInfoLog($"BUY Signal: CCI Slope {_currentSlope:F6} < Lower Threshold {lowerThreshold:F6}");
					}
					else if (_currentSlope > upperThreshold && Position >= 0)
					{
						...
						this.AddInfoLog($"SELL Signal: CCI Slope {_currentSlope:F6} > Upper Threshold {upperThreshold:F6}");
					}
					else if (_currentSlope > _averageSlope && Position > 0)
					{
						// Exit long position when slope returns to average (profit target)
						SellMarket(Position);
						this.AddInfoLog($"EXIT LONG: CCI Slope {_currentSlope:F6} returned to average {_averageSlope:F6}");
					}
					else if (_currentSlope < _averageSlope && Position < 0)
					{
						// Exit short position when slope returns to average (profit target)
						BuyMarket(Math.Abs(Position));
						this.AddInfoLog($"EXIT SHORT: CCI Slope {_currentSlope:F6} returned to average {_averageSlope:F6}");
					}
				}
			}
