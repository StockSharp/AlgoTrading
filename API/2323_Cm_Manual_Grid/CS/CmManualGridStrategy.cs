...
public class CmManualGridStrategy : Strategy
{
	...
	private void ProcessTrade(ITickTradeMessage t)
	{
		int bs = 0, ss = 0, bl = 0, sl = 0;
		decimal pbs = 0, pss = 0, pbl = 0, psl = 0, vbs = 0, vss = 0, vbl = 0, vsl = 0;
		foreach (var o in ActiveOrders)
		{
			if (o.Type == OrderTypes.BuyStop)
			{ bs++; if (o.Price > pbs) { pbs = o.Price; vbs = o.Volume; } }
			else if (o.Type == OrderTypes.SellStop)
			{ ss++; if (pss == 0 || o.Price < pss) { pss = o.Price; vss = o.Volume; } }
			else if (o.Type == OrderTypes.BuyLimit)
			{ bl++; if (pbl == 0 || o.Price < pbl) { pbl = o.Price; vbl = o.Volume; } }
			else if (o.Type == OrderTypes.SellLimit)
			{ sl++; if (o.Price > psl) { psl = o.Price; vsl = o.Volume; } }
		}
		var step = Security.PriceStep ?? 1m;
		if (bs < OrdersBuyStop)
			BuyStop(bs == 0 ? Lot : vbs + LotPlus, bs == 0 ? t.Price + FirstLevel * step : pbs + StepBuyStop * step);
		if (ss < OrdersSellStop)
			SellStop(ss == 0 ? Lot : vss + LotPlus, ss == 0 ? t.Price - FirstLevel * step : pss - StepSellStop * step);
		if (bl < OrdersBuyLimit)
			BuyLimit(bl == 0 ? Lot : vbl + LotPlus, bl == 0 ? t.Price - FirstLevel * step : pbl - StepBuyLimit * step);
		if (sl < OrdersSellLimit)
			SellLimit(sl == 0 ? Lot : vsl + LotPlus, sl == 0 ? t.Price + FirstLevel * step : psl + StepSellLimit * step);

		if (Position > 0 && PnL >= CloseProfitB)
			SellMarket(Position);
		else if (Position < 0 && PnL >= CloseProfitS)
			BuyMarket(-Position);

		if (PnL >= ProfitClose)
		{ CloseAll(); _trailHigh = 0m; return; }

		if (PnL >= TralStart && _trailHigh == 0m)
			_trailHigh = PnL;
		if (PnL > _trailHigh)
			_trailHigh = PnL;
		if (_trailHigh != 0m && PnL <= _trailHigh - TralClose)
		{ CloseAll(); _trailHigh = 0m; }
	}
