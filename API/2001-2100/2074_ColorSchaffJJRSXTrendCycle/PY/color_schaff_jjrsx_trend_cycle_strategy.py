...
class color_schaff_jjrsx_trend_cycle_strategy(Strategy):
    ...
    def OnReseted(self):
        super(color_schaff_jjrsx_trend_cycle_strategy, self).OnReseted()
        self._prev_stc = None

    def OnStarted2(self, time):
        super(color_schaff_jjrsx_trend_cycle_strategy, self).OnStarted2(time)
    ...
    def process_candle(self, candle, stc_val):
        ...
        if self._prev_stc is None:
            self._prev_stc = stc_val
            return

        if self._prev_stc <= float(self.high_level) and stc_val > float(self.high_level) and self.Position <= 0:
            if self.Position < 0:
                self.BuyMarket()
            self.BuyMarket()
        elif self._prev_stc >= float(self.low_level) and stc_val < float(self.low_level) and self.Position >= 0:
            if self.Position > 0:
                self.SellMarket()
            self.SellMarket()
    ...
    def CreateClone(self):
        return color_schaff_jjrsx_trend_cycle_strategy()
