...
class varanormal_mac_n_cheez_strategy(Strategy):
    def __init__(self):
        super(varanormal_mac_n_cheez_strategy, self).__init__()
        ...
        self._daily_target = self.Param("DailyTarget", 500) \
            .SetDisplay("Daily Target", "Daily profit target", "Risk")
        self._stop_loss_amount = self.Param("StopLossAmount", 200) \
            .SetDisplay("Stop Loss", "Stop loss amount", "Risk")
        self._trail_offset = self.Param("TrailOffset", DataType.TimeFrame(TimeSpan.FromMinutes(5))) \
    ...
    def daily_target(self):
        return self._daily_target.Value

    @property
    def stop_loss_amount(self):
        return self._stop_loss_amount.Value
    ...
    def OnReseted(self):
        super(varanormal_mac_n_cheez_strategy, self).OnReseted()
        self._prev_rsi = 0.0
        self._prev_fast = 0.0
        self._prev_slow = 0.0
        self._cooldown = 0
    ...
    def OnStarted2(self, time):
        super(varanormal_mac_n_cheez_strategy, self).OnStarted2(time)
    ...
    def CreateClone(self):
        return varanormal_mac_n_cheez_strategy()
