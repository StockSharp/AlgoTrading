...
class keltner_channel_strategy(Strategy):
    ...
    def OnProcess(self, candle, kc_value, ema_fast_value, ema_slow_value, ema_trend_value, atr_value):
        ...
        lower = float(lower)
        middle = float(middle)
        ema_fast = float(ema_fast_value.ToDecimal())
        ema_slow = float(ema_slow_value.ToDecimal())
        ema_trend = float(ema_trend_value.ToDecimal())
        atr_v = float(atr_value.ToDecimal())
