...
class status_mail_and_alert_on_order_close_strategy(Strategy):
    ...
    def OnStarted(self, time):
        super(status_mail_and_alert_on_order_close_strategy, self).OnStarted(time)
    ...
    def on_process(self, candle, fast, slow):
        ...
        if not self._has_prev:
            ...
            { if (self.Position < 0) BuyMarket(); if (self.Position <= 0) BuyMarket(); }
        elif self._prev_fast >= self._prev_slow and fast < slow:
            { if (self.Position > 0) SellMarket(); if (self.Position >= 0) SellMarket(); }
