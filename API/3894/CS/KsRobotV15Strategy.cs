...
/// <summary>
/// MetaTrader 4 "KSRobot 1.5" conversion that relies on the Ichimoku Kijun-sen line.
/// The strategy waits for price to cross the Kijun while a 20-period LWMA confirms the slope and distance filter.
/// Risk is managed via time windows, stop-loss, break-even and trailing logic reproduced from the original expert.
/// </summary>
public class KsRobotV15Strategy : Strategy
{
	...
	/// <summary>
	/// Initialize <see cref="KsRobotV15Strategy"/>.
	/// </summary>
...
// Disable automatic protective orders; the MT4 logic is implemented manually below.
...
// Reproduce the MT4 management rules before looking for a fresh entry.
ManageOpenPosition(candle, maCurrent);

if (IsFormedAndOnlineAndAllowTrading() && IsWithinTradingHours(candle.OpenTime))
{
// Evaluate price/indicator relationship to create new entry setups.
EvaluateEntrySignals(candle, kijun, maCurrent);
}

// Keep a compact history of values for the next bar calculations.
...
if (Position == 0)
{
// If the position was closed externally, clear the cached state.
...
{
// Capture the entry price once StockSharp reports the filled position.
...
{
// Synchronize cached protection levels if the average price changes (e.g. after partial fills).
...
{
// LWMA reversed before breakeven was reached, mimic the emergency exit from MT4.
...
if (!isLong && _stopLossPrice.Value > entryPrice && _previousMa.Value > _previousPrevMa.Value)
{
// Same emergency exit for shorts when the LWMA turns higher.
ClosePositionAndReset();
return;
}
}
...
if (isLong)
{
// Apply break-even and trailing adjustments before checking barriers.
...
{
// Mirror the protective logic for short exposure.
...
// Translate the MT4 "cross" flags into candle-level conditions.
...
{
BuyMarket(volume);
// Cache entry state immediately to allow break-even/trailing logic on the next bar.
...
{
SellMarket(volume);
// Mirror the same cache update for short positions.
...
// Convert pip-based settings to absolute price distances only once per trade.
